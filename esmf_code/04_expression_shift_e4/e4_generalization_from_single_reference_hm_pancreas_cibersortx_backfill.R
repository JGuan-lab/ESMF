# ============================================================================
# Ablation_Study_GeneralizationFromSingleReference_Hm_Pancreas_E4_CIBERSORTx_backfill.R
#
# 用途：补算 E4 表达偏移实验（Hm_Pancreas，人胰腺）中 CIBERSORTx 的响应指标。
#
# 背景：
#   - E4 v3 脚本（Ablation_Study_GeneralizationFromSingleReference_Hm_Pancreas_E4_ExpressionShift_v3.R）
#     已把 CIBERSORTx 输入导出到 CIBERSORTx_input_E4/，结果读取是注释占位。
#   - 现在 CIBERSORTx 结果 CSV 已跑出，本脚本按原始实验的 CIBERSORTx 结算逻辑
#     逐档补算 H_RMSE / H_Pearson。
#
# 细胞类型：Acinar, Alpha, Beta（3 种）
#
...
for (i in seq_along(shift_grid)) {
  ...
  fA <- file.path(CIBX_DIR, paste0("E4_Pancreas_scenarioA_shift", sn, "_Results.csv"))
  if (file.exists(fA)) {
  ...
  fB <- file.path(CIBX_DIR, paste0("E4_Pancreas_scenarioB_shift", sn, "_Results.csv"))
  if (file.exists(fB)) {
