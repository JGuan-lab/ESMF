# ============================================================================
# Ablation_Study_GeneralizationFromSingleReference_Mm_Brain_E4_ExpressionShift_v3.R
#
...
# MuSiC 的 cellType 标注（沿用原脚本 phenoDataC 处理：改名 + rownames = cellID）
pDataC_music <- pDataC
...
# 统一偏移函数：非负截断 + 四舍五入（保留矩阵维度）
# 保护：若截断后某基因行全 0（低表达基因被强噪声整体清零），恢复该行原值，
#       避免 NMF（DSA/CellMix::ged）报 "null row" 错误。
...
write_ciber(C_orig, file.path(cibx_dir, "E4_Brain_scenarioA_signature_matrix.txt"))
# 场景 B：mixture 不变（T_orig），signature 5 档（C 加偏）
write_ciber(T_orig, file.path(cibx_dir, "E4_Brain_scenarioB_mixture_file.txt"))
...
for (sh in shift_grid) {
  ...
  write_ciber(T_A, file.path(cibx_dir, paste0("E4_Brain_scenarioA_mixture_shift", sh_name, ".txt")))
  # 场景 B signature（C 加偏）
  C_B <- shift_nonneg(C_orig, sh, scale_C, eps_c_cibx)
  write_ciber(C_B, file.path(cibx_dir, paste0("E4_Brain_scenarioB_signature_shift", sh_name, ".txt")))
}
...
#       命名约定：
#         E4_Brain_scenarioA_shift<档>_Results.txt
#         E4_Brain_scenarioB_shift<档>_Results.txt
...
#   sh_name <- gsub("\\.", "_", as.character(sh))
#   # 场景 A
#   fA <- file.path(cibx_dir, paste0("E4_Brain_scenarioA_shift", sh_name, "_Results.txt"))
#   if (file.exists(fA)) {
