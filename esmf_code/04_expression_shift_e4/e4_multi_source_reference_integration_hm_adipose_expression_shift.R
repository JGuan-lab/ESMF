# ============================================================================
# Ablation_Study_GeneralizationFromSingleReference_Hm_Adipose_E4_ExpressionShift_v3.R
#
...
write_ciber(C_orig, file.path(cibx_dir, "E4_Adipose_scenarioA_signature_matrix.txt"))
# 场景 B：mixture 不变（T_orig），signature 5 档（C 加偏）
write_ciber(T_orig, file.path(cibx_dir, "E4_Adipose_scenarioB_mixture_file.txt"))
...
for (sh in shift_grid) {
  ...
  write_ciber(T_A, file.path(cibx_dir, paste0("E4_Adipose_scenarioA_mixture_shift", sh_name, ".txt")))
  # 场景 B signature（C 加偏）
  C_B <- shift_nonneg(C_orig, sh, scale_C, eps_c_cibx)
  write_ciber(C_B, file.path(cibx_dir, paste0("E4_Adipose_scenarioB_signature_shift", sh_name, ".txt")))
}
...
#       命名约定：
#         E4_Adipose_scenarioA_shift<档>_Results.txt
#         E4_Adipose_scenarioB_shift<档>_Results.txt
...
#   sh_name <- gsub("\\.", "_", as.character(sh))
#   # 场景 A
#   fA <- file.path(cibx_dir, paste0("E4_Adipose_scenarioA_shift", sh_name, "_Results.txt"))
#   if (file.exists(fA)) {
