# ============================================================================
# Export CIBERSORTx Input for E4 Expression Shift (v2) —— Brain
#
# 目的：导出 E4 偏移实验（场景 A 目标加噪 / 场景 B 参考加偏）所需的
#       CIBERSORTx 输入文件（signature matrix + mixture file），供手动网页运行。
# 逻辑：复用 Ablation_Study_crossPlatformConsistency_Mm_Brain_ParameterSensitivity_v2.R 的预处理，拿到 T_orig / C_orig / eps_g / scale_* 后，
...
sigA_file <- file.path(out_dir, "E4_Brain_scenarioA_signature_matrix.txt")
...
for (sh in shift_grid) {
  ...
  f <- file.path(out_dir, sprintf("E4_Brain_scenarioA_mixture_shift%s.txt",
...
mixB_file <- file.path(out_dir, "E4_Brain_scenarioB_mixture_file.txt")
...
for (sh in shift_grid) {
  ...
  f <- file.path(out_dir, sprintf("E4_Brain_scenarioB_signature_shift%s.txt",
                                   sub("\\.", "_", as.character(sh))))
  write_ciber(C_sh, f)
