...
analysis_prefix <- "GeneralizationFromSingleReference_Mm_Hypothalamus_6CT"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA FROM A SINGLE DATASET
# ============================================================================
# This section loads a mouse brain single-cell RNA-seq training dataset
# (SRA694785_SRS3214440) from the PanglaoDB database.
# Training dataset: SRA694785_SRS3214440
# Selected cell types: Astrocytes, Endothelialcells, Microglia, Neurons, Oligodendrocytes
# ============================================================================

# Load training dataset: SRA694785_SRS3214440
set <- "SRA694785_SRS3214440"

# Load annotation file containing cell type information
anno_SRA_data <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set), "_annotation.csv"),
                          sep = ",", header = FALSE)
colnames(anno_SRA_data) <- anno_SRA_data[1, ]
anno_SRA_data <- anno_SRA_data[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set), ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract cell type information from annotation
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_data[match(cellType_temp, anno_SRA_data$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for training data
phenoData_train <- data.frame(cellType = cellType_data, cellID = cellID_data)
rownames(phenoData_train) <- cellID_data

# Clean cell type names: remove spaces and hyphens
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub("-", "", phenoData_train$cellType)
...
# Load sparse expression matrix for training data
load(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set), ".sparse.RData"))
train_data <- sm
...
select_cellType <- gsub("Unknown", "", unique(phenoData_train$cellType))
...
# ============================================================================
# SECTION 4: LOAD AND PREPROCESS TEST DATA FROM MULTIPLE DATASETS
# ============================================================================
# This section loads and preprocesses five independent mouse brain single-cell
# RNA-seq test datasets from the PanglaoDB database for validation.
# Test datasets: SRA481537_SRS1724938, SRA481537_SRS1724927,
#                SRA481537_SRS1724932, SRA481537_SRS1724941, SRA592147_SRS2384617
# Selected cell types: Astrocytes, Endothelialcells, Microglia, Neurons, Oligodendrocytes
# ============================================================================

# ----------------------------------------------------------------------------
# Test Dataset 1: SRA481537_SRS1724938
# ----------------------------------------------------------------------------
test_set1 <- "SRA481537_SRS1724938"
...
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test1[match(cellType_temp, anno_SRA_test1$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for test data 1
...
# Load sparse expression matrix for test data 1
...
# ----------------------------------------------------------------------------
# Test Dataset 2: SRA481537_SRS1724927
# ----------------------------------------------------------------------------
test_set2 <- "SRA481537_SRS1724927"
...
# Extract cell type information
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test2[match(cellType_temp, anno_SRA_test2$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for test data 2
...
# Load sparse expression matrix for test data 2
...
# ----------------------------------------------------------------------------
# Test Dataset 3: SRA481537_SRS1724932
# ----------------------------------------------------------------------------
test_set3 <- "SRA481537_SRS1724932"
...
# Extract cell type information
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test3[match(cellType_temp, anno_SRA_test3$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for test data 3
...
# Load sparse expression matrix for test data 3
...
# ----------------------------------------------------------------------------
# Test Dataset 4: SRA481537_SRS1724941
# ----------------------------------------------------------------------------
test_set4 <- "SRA481537_SRS1724941"

# Load annotation file
anno_SRA_test4 <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set4), "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_test4) <- anno_SRA_test4[1, ]
anno_SRA_test4 <- anno_SRA_test4[-1, ]

# Load cluster assignments
meta_temp <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set4), ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract cell type information
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test4[match(cellType_temp, anno_SRA_test4$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for test data 4
phenoData_test4 <- data.frame(cellType = cellType_data, cellID = cellID_data)
rownames(phenoData_test4) <- cellID_data

# Clean cell type names
phenoData_test4$cellType <- sub(" ", "", phenoData_test4$cellType)
phenoData_test4$cellType <- sub(" ", "", phenoData_test4$cellType)
phenoData_test4$cellType <- sub(" ", "", phenoData_test4$cellType)
phenoData_test4$cellType <- sub(" ", "", phenoData_test4$cellType)
phenoData_test4$cellType <- sub("-", "", phenoData_test4$cellType)
table(phenoData_test4$cellType)
phenoData_test4$cellID <- paste0("test4", phenoData_test4$cellID)

# Load sparse expression matrix for test data 4
load(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set4), ".sparse.RData"))
test_data4 <- sm
rm(sm)
dim(test_data4)
colnames(test_data4) <- paste0("test4", colnames(test_data4))
table(phenoData_test4$cellType)

# Filter out "Unknown" cell types
select_cellType <- gsub("Unknown", "", unique(phenoData_test4$cellType))
phenoData_test4 <- phenoData_test4[which(phenoData_test4$cellType %in% select_cellType), ]
table(phenoData_test4$cellType)

# Ensure phenotype and expression data match
phenoData_test4 <- phenoData_test4[which(phenoData_test4$cellID %in% colnames(test_data4)), ]
table(phenoData_test4$cellType)
test_data4 <- test_data4[, intersect(phenoData_test4$cellID, colnames(test_data4))]
table(phenoData_test4$cellType)

# Convert to dense matrix and add metadata
test_data4 <- as(test_data4, "matrix")
phenoData_test4$name <- phenoData_test4$cellID
dim(test_data4)
dim(phenoData_test4)

# Standardize gene names
rownames(test_data4) <- unlist(lapply(strsplit(rownames(test_data4), "_"), function(x) x[1]))

# Merge with existing test data
rownames(test_data) <- toupper(rownames(test_data))
rownames(test_data4) <- toupper(rownames(test_data4))
test_data <- test_data[intersect(rownames(test_data), rownames(test_data4)), ]
test_data4 <- test_data4[intersect(rownames(test_data), rownames(test_data4)), ]
dim(test_data)
dim(test_data4)

test_data <- as.matrix(cbind(test_data, test_data4))
phenoData_test <- rbind(phenoData_test, phenoData_test4)
dim(test_data)
dim(phenoData_test)
table(phenoData_test$cellType)

# Remove temporary objects
rm(phenoData_test4, test_data4)

# ----------------------------------------------------------------------------
# Test Dataset 5: SRA592147_SRS2384617
# ----------------------------------------------------------------------------
test_set5 <- "SRA592147_SRS2384617"

# Load annotation file
anno_SRA_test5 <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set5), "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_test5) <- anno_SRA_test5[1, ]
anno_SRA_test5 <- anno_SRA_test5[-1, ]

# Load cluster assignments
meta_temp <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set5), ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract cell type information
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test5[match(cellType_temp, anno_SRA_test5$`Cluster ID`), 3, drop = FALSE]
cellType_data <- cellType_temp$`Inferred cell type`
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Create phenotype data frame for test data 5
phenoData_test5 <- data.frame(cellType = cellType_data, cellID = cellID_data)
rownames(phenoData_test5) <- cellID_data

# Clean cell type names
phenoData_test5$cellType <- sub(" ", "", phenoData_test5$cellType)
phenoData_test5$cellType <- sub(" ", "", phenoData_test5$cellType)
phenoData_test5$cellType <- sub(" ", "", phenoData_test5$cellType)
phenoData_test5$cellType <- sub(" ", "", phenoData_test5$cellType)
phenoData_test5$cellType <- sub("-", "", phenoData_test5$cellType)
table(phenoData_test5$cellType)
phenoData_test5$cellID <- paste0("test5", phenoData_test5$cellID)

# Load sparse expression matrix for test data 5
load(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set5), ".sparse.RData"))
test_data5 <- sm
rm(sm)
dim(test_data5)
colnames(test_data5) <- paste0("test5", colnames(test_data5))
table(phenoData_test5$cellType)

# Filter out "Unknown" cell types
select_cellType <- gsub("Unknown", "", unique(phenoData_test5$cellType))
phenoData_test5 <- phenoData_test5[which(phenoData_test5$cellType %in% select_cellType), ]
table(phenoData_test5$cellType)

# Ensure phenotype and expression data match
phenoData_test5 <- phenoData_test5[which(phenoData_test5$cellID %in% colnames(test_data5)), ]
table(phenoData_test5$cellType)
test_data5 <- test_data5[, intersect(phenoData_test5$cellID, colnames(test_data5))]
table(phenoData_test5$cellType)

# Convert to dense matrix and add metadata
test_data5 <- as(test_data5, "matrix")
phenoData_test5$name <- phenoData_test5$cellID
dim(test_data5)
dim(phenoData_test5)

# Standardize gene names
rownames(test_data5) <- unlist(lapply(strsplit(rownames(test_data5), "_"), function(x) x[1]))

# Merge with existing test data
rownames(test_data) <- toupper(rownames(test_data))
rownames(test_data5) <- toupper(rownames(test_data5))
test_data <- test_data[intersect(rownames(test_data), rownames(test_data5)), ]
test_data5 <- test_data5[intersect(rownames(test_data), rownames(test_data5)), ]
dim(test_data)
dim(test_data5)

test_data <- as.matrix(cbind(test_data, test_data5))
phenoData_test <- rbind(phenoData_test, phenoData_test5)
dim(test_data)
dim(phenoData_test)
table(phenoData_test$cellType)

# Remove temporary objects
rm(phenoData_test5, test_data5)

# ----------------------------------------------------------------------------
# Select Specific Cell Types for Analysis
# ----------------------------------------------------------------------------
# For this brain tissue analysis, we focus on five major neural cell types:
# Astrocytes, Endothelial cells, Microglia, Neurons, and Oligodendrocytes
select_cellType <- c("Astrocytes", "Endothelialcells", "Microglia", "Neurons", "Oligodendrocyteprogenitorcells", "Oligodendrocytes")
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_sim56_Results.csv", 
                      header = TRUE, sep = ",")
