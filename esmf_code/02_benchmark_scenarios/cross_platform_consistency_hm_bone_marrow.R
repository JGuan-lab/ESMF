...
analysis_prefix <- "crossPlatformConsistency_Hm_Bone-Marrow_threshold200"
...
normalization_scT = "TMM"
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA FROM HUSCH BONE MARROW DATASET
# ============================================================================
# This section loads and preprocesses the training dataset from the HUSCH database.
# The data is derived from human bone marrow (Single-cell RNA-seq).
# Training dataset: HU_0115_Bone-Marrow_GSE130756
# Selected cell types: B, Mono_Macro, Neutrophil
# ============================================================================

# Define the path to the training dataset directory
train_dir <- "/media/desk16/tjn050/LSC/HUSCH/Bone-Marrow/HU_0115_Bone-Marrow_GSE130756/"
train_set <- strsplit(train_dir, "/")[[1]][8]  # Extracts dataset identifier 'HU_0115_Bone-Marrow_GSE130756'
cat("Loading training dataset:", train_set, "\n")

# ----------------------------------------------------------------------------
# Step 3.1: Load cell metadata (phenotype data)
# ----------------------------------------------------------------------------
phenoData_train <- read.csv(paste0(train_dir, train_set, "_meta.txt"),
                            sep = "\t",
                            header = TRUE)

# Standardize cell type names: replace special characters for consistency
phenoData_train$Celltype <- sub("/", "_", phenoData_train$Celltype)
phenoData_train$Celltype <- sub(" ", "_", phenoData_train$Celltype)
phenoData_train$Celltype <- sub(" ", "_", phenoData_train$Celltype)  # Repeated to handle multiple spaces

# Rename columns to match the expected variable names in the downstream workflow
...
# ----------------------------------------------------------------------------
# Step 3.2: Load expression matrix (stored in HDF5 format)
# ----------------------------------------------------------------------------
library(rhdf5)
train_mat <- h5read(paste0(train_dir, train_set, "_gene_count.h5"), "matrix")

# Extract components from the HDF5 object
...
# Reconstruct a sparse matrix (dgCMatrix format) from the CSC (Compressed Sparse Column) indices
sparse_train_mat <- sparseMatrix(
...
colnames(sparse_train_mat) <- barcodes
rownames(sparse_train_mat) <- gene_id

# Convert the sparse matrix to a standard dense matrix for compatibility with downstream functions
train_data <- as.matrix(sparse_train_mat)
rm(sparse_train_mat)  # Remove the temporary sparse object to free memory
gc()                  # Trigger garbage collection

# ----------------------------------------------------------------------------
# Step 3.3: Align phenotype data with expression matrix
# ----------------------------------------------------------------------------
# Add a prefix to cell identifiers to distinguish training cells
colnames(train_data) <- paste0("train", colnames(train_data))
phenoData_train$cellID <- paste0("train", phenoData_train$cellID)

# Ensure that the phenotype data only contains cells present in the expression matrix
phenoData_train <- phenoData_train[which(phenoData_train$cellID %in% colnames(train_data)), ]
cat("Cell type distribution after alignment:\n")
print(table(phenoData_train$cellType))

# Subset the expression matrix to include only the cells with valid phenotype information
train_data <- train_data[, intersect(phenoData_train$cellID, colnames(train_data))]

# ----------------------------------------------------------------------------
# Step 3.4: Select cell types of interest and finalize data objects
# ----------------------------------------------------------------------------
# Define the cell types to retain for the deconvolution analysis
select_cellType <- c("B", "Mono_Macro", "Neutrophil")

phenoData_train <- phenoData_train[which(phenoData_train$cellType %in% select_cellType), ]
phenoData_train <- phenoData_train[which(phenoData_train$cellID %in% colnames(train_data)), ]
train_data <- train_data[, intersect(phenoData_train$cellID, colnames(train_data))]

# Ensure the expression matrix is in a standard matrix format
train_data <- as(train_data, "matrix")

# Create a 'name' column (required by some downstream functions) and reorder columns
phenoData_train$name <- phenoData_train$cellID
phenoData_train <- phenoData_train[, c("cellType", "cellID", "name")]  # Order: cellType, cellID, name

cat("Final training data dimensions:\n")
cat("  Expression matrix:", dim(train_data)[1], "genes x", dim(train_data)[2], "cells\n")
cat("  Phenotype data:", dim(phenoData_train)[1], "cells\n")
cat("  Cell type distribution:\n")
print(table(phenoData_train$cellType))


# ============================================================================
# SECTION 4: LOAD AND PREPROCESS TEST DATA FROM HUSCH BONE MARROW DATASET
# ============================================================================
# This section loads and preprocesses the independent test dataset from the HUSCH database.
# The data is derived from a different study on human bone marrow, used for validation.
# Test dataset: HU_0123_Bone-Marrow_GSE134355
# Selected cell types: B, Mono_Macro, Neutrophil (must match training set)
# ============================================================================

# Define the path to the test dataset directory
test_dir <- "/media/desk16/tjn050/LSC/HUSCH/Bone-Marrow/HU_0123_Bone-Marrow_GSE134355/"
test_set <- strsplit(test_dir, "/")[[1]][8]  # Extracts dataset identifier 'HU_0123_Bone-Marrow_GSE134355'
cat("\nLoading test dataset:", test_set, "\n")

# ----------------------------------------------------------------------------
# Step 4.1: Load cell metadata (phenotype data) for the test set
# ----------------------------------------------------------------------------
phenoData_test <- read.csv(paste0(test_dir, test_set, "_meta.txt"),
                           sep = "\t",
                           header = TRUE)

# Standardize cell type names (same procedure as for training data)
...
# ----------------------------------------------------------------------------
# Step 4.2: Load expression matrix (HDF5 format) for the test set
# ----------------------------------------------------------------------------
test_mat <- h5read(paste0(test_dir, test_set, "_gene_count.h5"), "matrix")

# Extract components and reconstruct sparse matrix
...
colnames(sparse_test_mat) <- barcodes
rownames(sparse_test_mat) <- gene_id

test_data <- as.matrix(sparse_test_mat)
rm(sparse_test_mat)
gc()

# ----------------------------------------------------------------------------
# Step 4.3: Align phenotype data with expression matrix (test set)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# Step 4.4: Select the same cell types of interest and finalize test data objects
# ----------------------------------------------------------------------------
# Use the same 'select_cellType' vector defined in Section 3
...
phenoData_test <- phenoData_test[, c("cellType", "cellID", "name")]  # Consistent column order

cat("Final test data dimensions:\n")
cat("  Expression matrix:", dim(test_data)[1], "genes x", dim(test_data)[2], "cells\n")
cat("  Phenotype data:", dim(phenoData_test)[1], "cells\n")
cat("  Cell type distribution:\n")
print(table(phenoData_test$cellType))

# ----------------------------------------------------------------------------
# Step 4.5: Standardize gene identifiers and integrate training and test data
# ----------------------------------------------------------------------------
# Standardize gene symbols: use the part before the first underscore and convert to uppercase
...
# Keep only the genes that are common to both the training and test datasets
...
cat("\nData dimensions after gene intersection:\n")
cat("  Training data:", dim(train_data)[1], "genes x", dim(train_data)[2], "cells\n")
cat("  Test data:", dim(test_data)[1], "genes x", dim(test_data)[2], "cells\n")

# Create a combined expression matrix and phenotype data frame for downstream QC and analysis
data <- as.matrix(cbind(train_data, test_data))
full_phenoData <- rbind(phenoData_train, phenoData_test)

cat("\nIntegrated dataset ready for downstream analysis:\n")
cat("  Combined expression matrix:", dim(data)[1], "genes x", dim(data)[2], "cells\n")
cat("  Combined phenotype data:", dim(full_phenoData)[1], "cells\n")
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/sim21_CIBERSORTx_Job135_Results.csv", 
                      header = TRUE, sep = ",")
