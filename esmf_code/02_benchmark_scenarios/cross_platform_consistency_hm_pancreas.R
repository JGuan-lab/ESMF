...
analysis_prefix <- "crossPlatformConsistency_Hm_Pancreas"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS PANCREAS TRAINING DATA (PanglaoDB)
# ============================================================================
# This section loads and preprocesses mouse pancreas single-cell RNA-seq data
# from the PanglaoDB database (SRA701877_SRS3279695) as the training reference.
# Cell types of interest: Acinar cells, Alpha cells, Beta cells, Ductal cells.
# ============================================================================

# Define the training dataset identifier
training_set <- "SRA701877_SRS3279695"

# ----------------------------------------------------------------------------
# Step 3.1: Load Cell Type Annotation Data
# ----------------------------------------------------------------------------
# Read the annotation file containing cluster-to-cell-type mappings
anno_SRA_data <- read.csv(
  paste0("/media/desk16/tjn050/LSC/PanglaoDB-master/PanglaoDB-master/data/annotation/", 
         training_set, "_annotation.csv"),
  sep = ",",
  header = FALSE
)
...
# ----------------------------------------------------------------------------
# Step 3.2: Load Cluster Assignment File
# ----------------------------------------------------------------------------
meta_temp <- read.csv(
  paste0("/media/desk16/tjn050/LSC/PanglaoDB-master/PanglaoDB-master/data/sample_clusters/", 
         training_set, ".clusters.txt"),
  sep = "\t",
  header = FALSE
)
...
# ----------------------------------------------------------------------------
# Step 3.3: Map Cluster IDs to Cell Types
# ----------------------------------------------------------------------------
# Extract cluster ID from cellID string
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
# Match cluster ID to annotation data to get cell type
...
# Clean cellID by removing numbers and spaces
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# ----------------------------------------------------------------------------
# Step 3.4: Create Phenotype Data Frame for Training Data
# ----------------------------------------------------------------------------
...
phenoData_train$cellType <- gsub("-", "", phenoData_train$cellType)
# Add prefix to cell IDs to identify training set
...
# ----------------------------------------------------------------------------
# Step 3.5: Load Sparse Expression Matrix
# ----------------------------------------------------------------------------
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB-master/PanglaoDB-master/data/mat/", 
            training_set, ".sparse.RData"))
train_data <- sm
# Remove temporary object to free memory
rm(sm)

# Check dimensions and add training prefix to column names
dim(train_data)
colnames(train_data) <- paste0("train", colnames(train_data))
table(phenoData_train$cellType)

# ----------------------------------------------------------------------------
# Step 3.6: Filter for Specific Pancreatic Cell Types
# ----------------------------------------------------------------------------
# Define cell types of interest for pancreas analysis
select_cellType <- c("Acinarcells", "Alphacells", "Betacells", "Ductalcells")

# Filter phenotype and expression data to selected cell types
phenoData_train <- phenoData_train[which(phenoData_train$cellType %in% select_cellType), ]
table(phenoData_train$cellType)

# Ensure phenotype and expression data are properly aligned
...
# ----------------------------------------------------------------------------
# Step 3.7: Finalize Training Data Structure
# ----------------------------------------------------------------------------
# Convert sparse matrix to dense matrix and add cell name metadata
train_data <- as(train_data, "matrix")
phenoData_train$name <- phenoData_train$cellID

# Display final training data dimensions
dim(train_data)
dim(phenoData_train)

# ============================================================================
# SECTION 4: LOAD AND PREPROCESS PANCREAS TEST DATA (HUSCH)
# ============================================================================
# This section loads and preprocesses independent mouse pancreas single-cell data
# from the HUSCH database (HU_0231_Pancreas_GSE134355) as the test/validation set.
# Cell types are harmonized with the training data for consistent analysis.
# ============================================================================

# Define test dataset directory and identifier
test_dir <- "/media/desk16/tjn050/LSC/HUSCH/Pancreas/HU_0231_Pancreas_GSE134355/"
test_set <- strsplit(test_dir, "/")[[1]][8]
test_set

# ----------------------------------------------------------------------------
# Step 4.1: Load Test Metadata
# ----------------------------------------------------------------------------
phenoData_test <- read.csv(
  paste0(test_dir, test_set, "_meta.txt"),
  sep = "\t",
  header = TRUE
)

# ----------------------------------------------------------------------------
# Step 4.2: Load Test Expression Data from H5 File
# ----------------------------------------------------------------------------
library(rhdf5)
test_mat <- h5read(paste0(test_dir, test_set, "_gene_count.h5"), "matrix")

# Clean cell type names in metadata: replace problematic characters
...
# ----------------------------------------------------------------------------
# Step 4.3: Reconstruct Sparse Matrix from H5 Components
# ----------------------------------------------------------------------------
...
# Build sparse matrix using Matrix package
library(Matrix)
mat <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
...
# Convert to dense matrix and add test prefix to column names
...
# ----------------------------------------------------------------------------
# Step 4.4: Align Test Data with Selected Cell Types
# ----------------------------------------------------------------------------
# Ensure phenotype and expression data are properly aligned
...
# ----------------------------------------------------------------------------
# Step 4.5: Harmonize Cell Type Naming Between Training and Test Data
# ----------------------------------------------------------------------------
# Standardize test data cell type names to match training data conventions
phenoData_test$cellType <- sub("Acinar", "Acinarcells", phenoData_test$cellType)
phenoData_test$cellType <- sub("Alpha", "Alphacells", phenoData_test$cellType)
phenoData_test$cellType <- sub("Beta", "Betacells", phenoData_test$cellType)
phenoData_test$cellType <- sub("Duct", "Ductalcells", phenoData_test$cellType)

# Filter test data to the same cell types as training data
select_cellType <- c("Acinarcells", "Alphacells", "Betacells", "Ductalcells")
...
# ----------------------------------------------------------------------------
# Step 4.6: Finalize Test Data Structure
# ----------------------------------------------------------------------------
# Convert to dense matrix and reorganize metadata columns
test_data <- as(test_data, "matrix")
phenoData_test$name <- phenoData_test$cellID
# Reorder columns to match training data structure: cellType, cellID, name
...
# ============================================================================
# DATA INTEGRATION: COMBINE TRAINING AND TEST DATASETS
# ============================================================================
# This section integrates training and test data for downstream deconvolution
# analysis, ensuring gene name consistency and proper data alignment.
# ============================================================================

# ----------------------------------------------------------------------------
# Step 4.7: Standardize Gene Names Across Datasets
# ----------------------------------------------------------------------------
# Extract primary gene symbols (before underscore) and convert to uppercase
...
# ----------------------------------------------------------------------------
# Step 4.8: Create Combined Data Matrix and Phenotype Data
# ----------------------------------------------------------------------------
# Combine expression matrices and phenotype data for full analysis
data <- as.matrix(cbind(train_data, test_data))
full_phenoData <- rbind(phenoData_train, phenoData_test)

# Display final combined data dimensions and cell type distribution
dim(data)
dim(full_phenoData)
...
# ============================================================================
# OUTPUT VARIABLES (consistent with original SECTION 3 and SECTION 4)
# ============================================================================
# The following variables are now available for downstream analysis:
# - data: Combined expression matrix (genes × cells)
# - full_phenoData: Combined phenotype data for all cells
# - phenoData_train: Training set phenotype data
# - train_data: Training set expression matrix
# - phenoData_test: Test set phenotype data  
# - test_data: Test set expression matrix
# These variables maintain identical names and structures as the original
# script sections, ensuring compatibility with subsequent analysis steps.
# ============================================================================
...
##sim24 SRA701877_SRS3279695/HU_0231_Pancreas_GSE134355
prio_markers=list(
  Acinarcells=c(     "CTRB2",
                     "REG1A",
                     "SERPINA3",
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_SRA701877_SRS3279695_HU_0231_Pancreas_GSE134355.csv", 
                      header = TRUE, sep = ",")
