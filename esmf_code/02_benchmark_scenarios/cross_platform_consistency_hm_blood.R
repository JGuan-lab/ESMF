...
analysis_prefix <- "crossPlatformConsistency_Hm_Blood"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA FROM A SINGLE PANGLADB DATASET
# ============================================================================
# This section loads and preprocesses a single-cell RNA-seq training dataset
# from the PanglaoDB database (SRA749327_SRS3693913). The dataset contains
# immune cell types from an unspecified tissue/organ, intended for deconvolution
# benchmarking. Selected cell types: B cells, Dendritic cells, NK cells.
# ============================================================================

# Define the identifier for the training dataset
train_set = "SRA749327_SRS3693913"

# Load the cluster annotation file containing cell type information
anno_SRA_data <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, "_annotation.csv"),
                          sep = ",", header = FALSE)
colnames(anno_SRA_data) <- anno_SRA_data[1,]
anno_SRA_data <- anno_SRA_data[-1,]

# Load the file containing cluster assignments for each cell barcode
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract the numeric cluster ID from the cellID and map it to the annotated cell type
...
# Clean the original cell barcode by removing numbers and spaces to get a base ID
cellID_data <- gsub("[0-9]", "", meta_temp$cellID)
cellID_data <- gsub(" ", "", cellID_data)

# Construct the phenotype data frame containing cell type and cell ID
phenoData_train <- data.frame(cellType = cellType_data, cellID = cellID_data)
rownames(phenoData_train) <- cellID_data

# Standardize cell type names: remove all spaces and hyphens
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- sub("-", "", phenoData_train$cellType)
table(phenoData_train$cellType) # Check cell type distribution

# Prefix cell IDs to mark them as training data
phenoData_train$cellID <- paste0("train", phenoData_train$cellID)

# Load the sparse expression matrix and convert it to a standard R object
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, ".sparse.RData"))
train_data <- sm
rm(sm) # Remove the large sparse matrix object to free memory
dim(train_data)

# Prefix column names (cell barcodes) to mark them as training data
colnames(train_data) <- paste0("train", colnames(train_data))
table(phenoData_train$cellType)

# Define the cell types of interest for this analysis
select_cellType <- c("Bcells", "Dendriticcells", "NKcells")

# Subset the phenotype and expression data to retain only the selected cell types
phenoData_train <- phenoData_train[which(phenoData_train$cellType %in% select_cellType),]
table(phenoData_train$cellType)

# Ensure the phenotype data and expression matrix columns are synchronized
...
# Convert the sparse matrix to a standard dense matrix for downstream processing
train_data <- as(train_data, "matrix")
# Add a column for cell name (identical to cellID) for compatibility with some functions
phenoData_train$name <- phenoData_train$cellID
dim(train_data)
dim(phenoData_train)

# Standardize gene names: for genes with an underscore, use the first part as the official symbol
...
# ============================================================================
# This section loads and preprocesses an independent single-cell RNA-seq test dataset
# from the HUSCH collection (HU_0036_Blood_10x). This dataset serves as the
# independent validation set for deconvolution method benchmarking.
# The same immune cell types (B, Dendritic, NK cells) are selected.
# ============================================================================

# Define the path and identifier for the test dataset
library(rhdf5)
test_dir = "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0036_Blood_10x/"
test_set = strsplit(test_dir, "/")[[1]][8] # Extract dataset name from path
test_set

# Load the metadata (cell annotations) for the test dataset
phenoData_test <- read.csv(paste0(test_dir, test_set, "_meta.txt"), sep = "\t", header = TRUE)

# Standardize cell type names in the metadata: replace problematic characters
phenoData_test$Celltype <- sub("/", "_", phenoData_test$Celltype)
phenoData_test$Celltype <- sub(" ", "_", phenoData_test$Celltype)
phenoData_test$Celltype <- sub(" ", "_", phenoData_test$Celltype)
# Rename columns to match the expected format (cellID, cellType, Platform)
colnames(phenoData_test) = c("cellID", "cellType", "Platform")
table(phenoData_test$cellType)

# Load the gene expression count matrix stored in HDF5 format
test_mat <- h5read(paste0(test_dir, test_set, "_gene_count.h5"), "matrix")

# Extract components from the HDF5 structure for sparse matrix reconstruction
...
# Reconstruct the sparse matrix using the extracted components
...
test_data <- as.matrix(mat) # Convert to a standard dense matrix
rm(mat) # Remove the intermediate sparse matrix object

# Prefix cell barcodes to mark them as test data
colnames(test_data) <- paste0("test", colnames(test_data))
phenoData_test$cellID <- paste0("test", phenoData_test$cellID)
table(phenoData_test$cellType)

# Synchronize phenotype data with the expression matrix columns
...
# Further standardize and unify cell type nomenclature between training and test sets
phenoData_test$cellType <- sub("^B$", "Bcells", phenoData_test$cellType)
phenoData_test$cellType <- sub("^DC$", "Dendriticcells", phenoData_test$cellType)
phenoData_test$cellType <- sub("^NK$", "NKcells", phenoData_test$cellType)
table(phenoData_test$cellType)

# Subset the test data to the same cell types of interest
select_cellType <- c("Bcells", "Dendriticcells", "NKcells")
...
# Prepare the phenotype data frame with the required columns (cellType, cellID, name)
phenoData_test$name <- phenoData_test$cellID
phenoData_test <- phenoData_test[, c("cellType", "cellID", "name")] # Reorder columns
dim(test_data)
dim(phenoData_test)

# ============================================================================
# DATA INTEGRATION AND GENE NAME UNIFICATION
# ============================================================================
# This part unifies the training and test datasets by aligning gene symbols
# and merging them into a single combined expression matrix and phenotype table.

# Convert all gene symbols to uppercase to ensure case-insensitive matching
rownames(train_data) <- toupper(rownames(train_data))
rownames(test_data) <- toupper(rownames(test_data))

# Keep only the genes that are present in both the training and test datasets
...
# Combine the training and test expression matrices column-wise
data <- as.matrix(cbind(train_data, test_data))
# Combine the corresponding phenotype data rows
full_phenoData <- rbind(phenoData_train, phenoData_test)

# Final check of the dimensions and cell type distribution in the combined dataset
dim(data)
dim(full_phenoData)
...
# ============================================================================
# END OF MODIFIED DATA LOADING SECTIONS
# ============================================================================
# The variables `data`, `full_phenoData`, `train_data`, `test_data`,
# `phenoData_train`, and `phenoData_test` are now defined and can be
# directly used by the subsequent sections of the original pipeline
# (e.g., Quality Control in SECTION 5).
# ============================================================================
...
prio_markers=list(
  Bcells=c(     "FCER2","HLA-DRA","HLA-DQA1","CD24","IGHG4"),
  Dendriticcells=c("CD1A","CD1C","CD209","CD1B","HLA-DRA","HLA-DQA1","HLA-DMB","HLA-DMA","HLA-DQB1","HLA-DRB1","HLA-DPA1","HLA-DPB1","LILRB2","CXCL8","FCGR3A","FTL","SERPINA1","SIGLEC6","LYZ","C1ORF54","HLA-A","HLA-B","HLA-C","HLA-DRB5"),
  NKcells=c("FCGR3A","SIGLEC7","KLRF1","GNLY","IL32","GZMH","FGFBP2","XCL2","KIR2DL1","LILRB1","NCR3","LAIR2")
)
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_sim23_Results.csv", 
                      header = TRUE, sep = ",")
