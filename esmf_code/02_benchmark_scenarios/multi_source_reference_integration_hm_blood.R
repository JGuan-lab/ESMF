...
analysis_prefix <- "MultiSourceReferenceIntegration_Hm_Blood_threshold200"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA FROM HUSCH BLOOD DATASETS
# ============================================================================
# This section loads and preprocesses six human peripheral blood mononuclear cell
# (PBMC) single-cell RNA-seq datasets from the HUSCH database.
# All datasets share the same tissue (blood) and species (human) and are
# generated using the 10x Genomics platform.
# Training datasets: HU_0036, HU_0037, HU_0038, HU_0039, HU_0040, HU_0041
# Test dataset: HU_0044
# Selected cell types: B, CD4T, CD8T, Mono_Macro, NK
# ============================================================================

# ----------------------------------------------------------------------------
# Training Dataset 1: HU_0036_Blood_10x
# ----------------------------------------------------------------------------
train_dir1 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0036_Blood_10x/"
train_set1 <- basename(train_dir1)

# Load cell metadata
phenoData_train1 <- read.csv(paste0(train_dir1, train_set1, "_meta.txt"),
                             sep = "\t", header = TRUE)

# Standardize cell type names: replace spaces and forward slashes with underscores
phenoData_train1$Celltype <- gsub("/", "_", phenoData_train1$Celltype)
phenoData_train1$Celltype <- gsub(" ", "_", phenoData_train1$Celltype)
colnames(phenoData_train1) <- c("cellID", "cellType", "Platform")

# Load gene expression data from HDF5 file
library(rhdf5)
train_mat1 <- h5read(paste0(train_dir1, train_set1, "_gene_count.h5"), "matrix")

# Reconstruct sparse matrix from HDF5 components
barcodes <- train_mat1$barcodes
counts <- train_mat1$data
gene_id <- train_mat1$features$id
indices <- train_mat1$indices
indptr <- train_mat1$indptr
shape <- train_mat1$shape

library(Matrix)
train_data1 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data1) <- barcodes
rownames(train_data1) <- gene_id
train_data1 <- as.matrix(train_data1)  # Convert to dense matrix for compatibility

# Clean up large temporary objects
rm(train_mat1, barcodes, counts, gene_id, indices, indptr, shape)

# Add dataset-specific prefix to cell identifiers to ensure uniqueness
colnames(train_data1) <- paste0("train1", colnames(train_data1))
phenoData_train1$cellID <- paste0("train1", phenoData_train1$cellID)

# Ensure phenotype and expression data are aligned
phenoData_train1 <- phenoData_train1[phenoData_train1$cellID %in% colnames(train_data1), ]
...
# ----------------------------------------------------------------------------
# Training Dataset 2: HU_0037_Blood_10x
# ----------------------------------------------------------------------------
train_dir2 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0037_Blood_10x/"
train_set2 <- basename(train_dir2)

phenoData_train2 <- read.csv(paste0(train_dir2, train_set2, "_meta.txt"),
                             sep = "\t", header = TRUE)
phenoData_train2$Celltype <- gsub("/", "_", phenoData_train2$Celltype)
phenoData_train2$Celltype <- gsub(" ", "_", phenoData_train2$Celltype)
colnames(phenoData_train2) <- c("cellID", "cellType", "Platform")

train_mat2 <- h5read(paste0(train_dir2, train_set2, "_gene_count.h5"), "matrix")

barcodes <- train_mat2$barcodes
counts <- train_mat2$data
gene_id <- train_mat2$features$id
indices <- train_mat2$indices
indptr <- train_mat2$indptr
shape <- train_mat2$shape

train_data2 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data2) <- barcodes
rownames(train_data2) <- gene_id
train_data2 <- as.matrix(train_data2)

rm(train_mat2, barcodes, counts, gene_id, indices, indptr, shape)

colnames(train_data2) <- paste0("train2", colnames(train_data2))
phenoData_train2$cellID <- paste0("train2", phenoData_train2$cellID)

phenoData_train2 <- phenoData_train2[phenoData_train2$cellID %in% colnames(train_data2), ]
...
# ----------------------------------------------------------------------------
# Standardize gene symbols to uppercase for consistent merging
rownames(train_data1) <- toupper(rownames(train_data1))
rownames(train_data2) <- toupper(rownames(train_data2))

# Retain only genes present in both datasets
common_genes <- intersect(rownames(train_data1), rownames(train_data2))
train_data1 <- train_data1[common_genes, ]
train_data2 <- train_data2[common_genes, ]
...
# Remove individual dataset objects to free memory
rm(train_data1, train_data2, phenoData_train1, phenoData_train2)
...
# ----------------------------------------------------------------------------
# Training Dataset 3: HU_0038_Blood_10x
# ----------------------------------------------------------------------------
train_dir3 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0038_Blood_10x/"
train_set3 <- basename(train_dir3)

phenoData_train3 <- read.csv(paste0(train_dir3, train_set3, "_meta.txt"),
                             sep = "\t", header = TRUE)
phenoData_train3$Celltype <- gsub("/", "_", phenoData_train3$Celltype)
phenoData_train3$Celltype <- gsub(" ", "_", phenoData_train3$Celltype)
colnames(phenoData_train3) <- c("cellID", "cellType", "Platform")

train_mat3 <- h5read(paste0(train_dir3, train_set3, "_gene_count.h5"), "matrix")

barcodes <- train_mat3$barcodes
counts <- train_mat3$data
gene_id <- train_mat3$features$id
indices <- train_mat3$indices
indptr <- train_mat3$indptr
shape <- train_mat3$shape

train_data3 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data3) <- barcodes
rownames(train_data3) <- gene_id
train_data3 <- as.matrix(train_data3)

rm(train_mat3, barcodes, counts, gene_id, indices, indptr, shape)

colnames(train_data3) <- paste0("train3", colnames(train_data3))
phenoData_train3$cellID <- paste0("train3", phenoData_train3$cellID)

phenoData_train3 <- phenoData_train3[phenoData_train3$cellID %in% colnames(train_data3), ]
...
common_genes <- intersect(rownames(train_data), rownames(train_data3))
train_data <- train_data[common_genes, ]
train_data3 <- train_data3[common_genes, ]

# Merge with existing combined data
train_data <- as.matrix(cbind(train_data, train_data3))
phenoData_train <- rbind(phenoData_train, phenoData_train3)
rm(train_data3, phenoData_train3)
...
# ----------------------------------------------------------------------------
# Training Dataset 4: HU_0039_Blood_10x
# ----------------------------------------------------------------------------
train_dir4 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0039_Blood_10x/"
train_set4 <- basename(train_dir4)

phenoData_train4 <- read.csv(paste0(train_dir4, train_set4, "_meta.txt"),
                             sep = "\t", header = TRUE)
phenoData_train4$Celltype <- gsub("/", "_", phenoData_train4$Celltype)
phenoData_train4$Celltype <- gsub(" ", "_", phenoData_train4$Celltype)
colnames(phenoData_train4) <- c("cellID", "cellType", "Platform")

train_mat4 <- h5read(paste0(train_dir4, train_set4, "_gene_count.h5"), "matrix")

barcodes <- train_mat4$barcodes
counts <- train_mat4$data
gene_id <- train_mat4$features$id
indices <- train_mat4$indices
indptr <- train_mat4$indptr
shape <- train_mat4$shape

train_data4 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data4) <- barcodes
rownames(train_data4) <- gene_id
train_data4 <- as.matrix(train_data4)

rm(train_mat4, barcodes, counts, gene_id, indices, indptr, shape)

colnames(train_data4) <- paste0("train4", colnames(train_data4))
phenoData_train4$cellID <- paste0("train4", phenoData_train4$cellID)

phenoData_train4 <- phenoData_train4[phenoData_train4$cellID %in% colnames(train_data4), ]
...
common_genes <- intersect(rownames(train_data), rownames(train_data4))
train_data <- train_data[common_genes, ]
train_data4 <- train_data4[common_genes, ]

# Merge with existing combined data
train_data <- as.matrix(cbind(train_data, train_data4))
phenoData_train <- rbind(phenoData_train, phenoData_train4)
rm(train_data4, phenoData_train4)
...
# ----------------------------------------------------------------------------
# Training Dataset 5: HU_0040_Blood_10x
# ----------------------------------------------------------------------------
train_dir5 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0040_Blood_10x/"
train_set5 <- basename(train_dir5)

phenoData_train5 <- read.csv(paste0(train_dir5, train_set5, "_meta.txt"),
                             sep = "\t", header = TRUE)
phenoData_train5$Celltype <- gsub("/", "_", phenoData_train5$Celltype)
phenoData_train5$Celltype <- gsub(" ", "_", phenoData_train5$Celltype)
colnames(phenoData_train5) <- c("cellID", "cellType", "Platform")

train_mat5 <- h5read(paste0(train_dir5, train_set5, "_gene_count.h5"), "matrix")

barcodes <- train_mat5$barcodes
counts <- train_mat5$data
gene_id <- train_mat5$features$id
indices <- train_mat5$indices
indptr <- train_mat5$indptr
shape <- train_mat5$shape

train_data5 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data5) <- barcodes
rownames(train_data5) <- gene_id
train_data5 <- as.matrix(train_data5)

rm(train_mat5, barcodes, counts, gene_id, indices, indptr, shape)

colnames(train_data5) <- paste0("train5", colnames(train_data5))
phenoData_train5$cellID <- paste0("train5", phenoData_train5$cellID)

phenoData_train5 <- phenoData_train5[phenoData_train5$cellID %in% colnames(train_data5), ]
...
common_genes <- intersect(rownames(train_data), rownames(train_data5))
train_data <- train_data[common_genes, ]
train_data5 <- train_data5[common_genes, ]
...
# ----------------------------------------------------------------------------
# Training Dataset 6: HU_0041_Blood_10x
# ----------------------------------------------------------------------------
train_dir6 <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0041_Blood_10x/"
train_set6 <- basename(train_dir6)

phenoData_train6 <- read.csv(paste0(train_dir6, train_set6, "_meta.txt"),
                             sep = "\t", header = TRUE)
phenoData_train6$Celltype <- gsub("/", "_", phenoData_train6$Celltype)
phenoData_train6$Celltype <- gsub(" ", "_", phenoData_train6$Celltype)
colnames(phenoData_train6) <- c("cellID", "cellType", "Platform")

train_mat6 <- h5read(paste0(train_dir6, train_set6, "_gene_count.h5"), "matrix")

barcodes <- train_mat6$barcodes
counts <- train_mat6$data
gene_id <- train_mat6$features$id
indices <- train_mat6$indices
indptr <- train_mat6$indptr
shape <- train_mat6$shape

train_data6 <- sparseMatrix(
  i = indices[] + 1,
  p = indptr[],
  x = as.numeric(counts[]),
  dims = shape[],
...
colnames(train_data6) <- barcodes
rownames(train_data6) <- gene_id
train_data6 <- as.matrix(train_data6)

rm(train_mat6, barcodes, counts, gene_id, indices, indptr, shape)

colnames(train_data6) <- paste0("train6", colnames(train_data6))
phenoData_train6$cellID <- paste0("train6", phenoData_train6$cellID)

phenoData_train6 <- phenoData_train6[phenoData_train6$cellID %in% colnames(train_data6), ]
train_data6 <- train_data6[, intersect(phenoData_train6$cellID, colnames(train_data6))]

# Prepare for merging
rownames(train_data) <- toupper(rownames(train_data))
rownames(train_data6) <- toupper(rownames(train_data6))
common_genes <- intersect(rownames(train_data), rownames(train_data6))
train_data <- train_data[common_genes, ]
train_data6 <- train_data6[common_genes, ]

# Final merge of all six training datasets
train_data <- as.matrix(cbind(train_data, train_data6))
phenoData_train <- rbind(phenoData_train, phenoData_train6)
rm(train_data6, phenoData_train6)
...
# ============================================================================
# Load independent test dataset: HU_0044_Blood_10x
# This dataset serves as held-out validation for deconvolution performance.
# ============================================================================
test_dir <- "/media/desk16/tjn050/LSC/HUSCH/Blood/HU_0044_Blood_10x/"
test_set <- basename(test_dir)

phenoData_test <- read.csv(paste0(test_dir, test_set, "_meta.txt"),
                           sep = "\t", header = TRUE)
phenoData_test$Celltype <- gsub("/", "_", phenoData_test$Celltype)
phenoData_test$Celltype <- gsub(" ", "_", phenoData_test$Celltype)
...
colnames(test_data) <- barcodes
rownames(test_data) <- gene_id
test_data <- as.matrix(test_data)

rm(test_mat, barcodes, counts, gene_id, indices, indptr, shape)
...
# ----------------------------------------------------------------------------
# For this blood tissue analysis, we focus on five major immune cell types
select_cellType <- c("B", "CD4T", "CD8T", "Mono_Macro", "NK")
...
# Combine training and test data for downstream quality control and analysis
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/sim4_HU_0036_Blood_10x_CIBERSORTx_Job60_Results.csv", 
                      header = TRUE, sep = ",")
