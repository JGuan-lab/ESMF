...
analysis_prefix <- "GeneralizationFromSingleReference_Hm_Prostate_threshold200"
...
# ============================================================================
# This section loads and preprocesses mouse prostate single-cell RNA-seq data
# from the HUSCH database (GSE172316).
# Training dataset: HU_0377_Prostate_GSE172316
# Selected cell types: Basal, Endothelial, Smooth_Muscle
# ============================================================================

# ----------------------------------------------------------------------------
# Load HUSCH Training Dataset
# ----------------------------------------------------------------------------
train_dir <- "/media/desk16/tjn050/LSC/HUSCH/Prostate/HU_0377_Prostate_GSE172316/"
train_set <- strsplit(train_dir, "/")[[1]][8]

# Load phenotype data (cell annotations)
phenoData_train <- read.csv(
  paste0(train_dir, train_set, "_meta.txt"),
  sep = "\t",
  header = TRUE
)
...
library(rhdf5)
train_mat <- h5read(
  paste0(train_dir, train_set, "_gene_count.h5"),
  "matrix"
)

# Extract components from H5 file
...
# Convert to matrix format and prepare phenotype data
...
# ============================================================================
# This section loads and preprocesses three mouse prostate single-cell RNA-seq
# datasets from the PanglaoDB database for independent validation.
# Test datasets: SRA826293_SRS4181130, SRA742961_SRS3565197, SRA742961_SRS3565207
# Selected cell types: Basal, Endothelial, Smooth_Muscle
# ============================================================================

# ----------------------------------------------------------------------------
# Test Dataset 1: SRA826293_SRS4181130
# ----------------------------------------------------------------------------
test_set1 <- "SRA826293_SRS4181130"

# Load annotation file
anno_SRA_test1 <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set1), "_annotation.csv"),
  sep = ",",
  header = FALSE
)
...
meta_temp <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set1), ".clusters.txt"),
  sep = "\t",
  header = FALSE
)
...
# ----------------------------------------------------------------------------
# Test Dataset 2: SRA742961_SRS3565197
# ----------------------------------------------------------------------------
test_set2 <- "SRA742961_SRS3565197"

# Load annotation file
anno_SRA_test2 <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set2), "_annotation.csv"),
  sep = ",",
  header = FALSE
)
...
# Load cluster assignments
meta_temp <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set2), ".clusters.txt"),
  sep = "\t",
  header = FALSE
)
colnames(meta_temp) <- "cellID"
...
# ----------------------------------------------------------------------------
# Test Dataset 3: SRA742961_SRS3565207
# ----------------------------------------------------------------------------
test_set3 <- "SRA742961_SRS3565207"

# Load annotation file
anno_SRA_test3 <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set3), "_annotation.csv"),
  sep = ",",
  header = FALSE
)
...
# Load cluster assignments
meta_temp <- read.csv(
  paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set3), ".clusters.txt"),
  sep = "\t",
  header = FALSE
)
colnames(meta_temp) <- "cellID"
...
# ----------------------------------------------------------------------------
# Align cell type names between training and test datasets
table(phenoData_test$cellType)
table(phenoData_train$cellType)

# Rename test dataset cell types to match training data convention
phenoData_test$cellType <- sub("Basalcells", "Basal", phenoData_test$cellType)
phenoData_test$cellType <- sub("Endothelialcells", "Endothelial", phenoData_test$cellType)
phenoData_test$cellType <- sub("Smoothmusclecells", "Smooth_Muscle", phenoData_test$cellType)
...
# ----------------------------------------------------------------------------
# For this prostate tissue analysis, we focus on three major cell types:
# Basal, Endothelial, and Smooth_Muscle
select_cellType <- c("Basal", "Endothelial", "Smooth_Muscle")
...
prio_markers=list(
  ...
  Smooth_Muscle=c("ACTA2","MYL9","TAGLN")
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_sim53_Results.csv", 
                      header = TRUE, sep = ",")
