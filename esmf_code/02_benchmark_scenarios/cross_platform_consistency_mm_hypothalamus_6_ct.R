...
analysis_prefix <- "crossPlatformConsistency_Mm_Hypothalamus_5CT"
...
marker_strategy = "all"
number_cells = 80  # 6CT: lowered from 100 so Microglia (80 cells in test set) can be sampled
to_remove = "none"
...
method = "OLS"
number_cells = 80  # 6CT: no round(,-2) here — round(80,-2) would give 100
to_remove = "none"
...
# ============================================================================
# SECTION 3 & 4 REPLACEMENT: LOAD AND PREPROCESS USER-SPECIFIED TRAINING AND TEST DATA
# ============================================================================
# DESCRIPTION:
# This section loads and preprocesses the user-provided mouse brain single-cell
# RNA-seq datasets. It replaces the original multi-source training data with a
# single training dataset and uses a separate dataset for independent testing.
# The workflow ensures data integrity, performs basic filtering, and prepares
# the expression matrices and phenotype data for downstream deconvolution analysis.
#
# INPUT DATASETS (User-defined):
# Training Data: SRA694785_SRS3214440
# Test Data:     SRA481537_SRS1724938
#
# SELECTED CELL TYPES:
# Astrocytes, Neurons, Oligodendrocyteprogenitorcells, Oligodendrocytes
# ============================================================================

# ----------------------------------------------------------------------------
# Load Required Packages
# ----------------------------------------------------------------------------
# Ensure necessary packages are loaded for data manipulation
library(Matrix)
library(dplyr)

# ----------------------------------------------------------------------------
# Step 1: Load and Preprocess TRAINING Dataset (SRA694785_SRS3214440)
# ----------------------------------------------------------------------------
# Define the training dataset identifier
train_set <- "SRA694785_SRS3214440"

# Load the annotation file containing cluster-to-cellType mappings
train_anno <- read.csv(
  file = paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, "_annotation.csv"),
  sep = ",", header = FALSE, stringsAsFactors = FALSE
)
colnames(train_anno) <- train_anno[1, ]
train_anno <- train_anno[-1, ]

# Load the cluster assignment file for all cells
train_meta <- read.csv(
  file = paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, ".clusters.txt"),
  sep = "\t", header = FALSE, stringsAsFactors = FALSE
)
colnames(train_meta) <- "cellID"

# Extract the cluster ID (numeric part) from the cellID
cluster_id <- gsub("[^0-9]", "", train_meta$cellID)
# Map cluster ID to the inferred cell type from the annotation file
cellType_info <- train_anno[match(cluster_id, train_anno$`Cluster ID`), 3, drop = FALSE]
cellType_vector <- cellType_info$`Inferred cell type`

# Clean the original cellID by removing numbers and spaces
clean_cellID <- gsub("[0-9]", "", train_meta$cellID)
clean_cellID <- gsub(" ", "", clean_cellID)

# Create the phenotype data frame for the training set
phenoData_train <- data.frame(
  cellType = cellType_vector,
  cellID = clean_cellID,
  stringsAsFactors = FALSE
)
rownames(phenoData_train) <- clean_cellID

# Standardize cell type names: remove spaces and hyphens
phenoData_train$cellType <- gsub(" ", "", phenoData_train$cellType)
phenoData_train$cellType <- gsub("-", "", phenoData_train$cellType)

# Add a prefix to cellIDs to identify the training set
phenoData_train$cellID <- paste0("train", phenoData_train$cellID)
cat("Cell type distribution in raw training data:\n")
print(table(phenoData_train$cellType))

# Load the sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", train_set, ".sparse.RData"))
train_data <- sm  # 'sm' is the sparse matrix object loaded from the .RData file
rm(sm)  # Remove the temporary object to free memory
cat("Dimensions of raw training expression matrix:", dim(train_data), "\n")

# Add prefix to column names (cell barcodes) of the expression matrix
colnames(train_data) <- paste0("train", colnames(train_data))

# ----------------------------------------------------------------------------
# Step 2: Filter Training Data for Selected Cell Types
# ----------------------------------------------------------------------------
# Define the cell types of interest for brain tissue analysis
selected_cell_types <- c("Astrocytes", "Neurons", "Oligodendrocyteprogenitorcells", "Oligodendrocytes", "Endothelialcells")

# Subset phenotype data to keep only the selected cell types
phenoData_train <- phenoData_train[phenoData_train$cellType %in% selected_cell_types, ]
cat("Cell type distribution after selecting specific types:\n")
print(table(phenoData_train$cellType))

# Ensure phenotype entries have corresponding columns in the expression matrix
phenoData_train <- phenoData_train[phenoData_train$cellID %in% colnames(train_data), ]
train_data <- train_data[, intersect(phenoData_train$cellID, colnames(train_data))]

# Convert the sparse matrix to a dense matrix for downstream compatibility
train_data <- as(train_data, "matrix")
# Add a 'name' column identical to cellID for compatibility with some functions
phenoData_train$name <- phenoData_train$cellID

cat("Final dimensions of TRAINING data:\n")
cat("  Expression matrix (train_data):", dim(train_data), "\n")
cat("  Phenotype data (phenoData_train):", dim(phenoData_train), "\n")


# ----------------------------------------------------------------------------
# Step 3: Load and Preprocess TEST Dataset (SRA481537_SRS1724938)
# ----------------------------------------------------------------------------
# Define the test dataset identifier
test_set <- "SRA481537_SRS1724938"

# Load annotation file for the test set
test_anno <- read.csv(
  file = paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set, "_annotation.csv"),
  sep = ",", header = FALSE, stringsAsFactors = FALSE
)
colnames(test_anno) <- test_anno[1, ]
test_anno <- test_anno[-1, ]

# Load cluster assignments for the test set
test_meta <- read.csv(
  file = paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set, ".clusters.txt"),
  sep = "\t", header = FALSE, stringsAsFactors = FALSE
)
colnames(test_meta) <- "cellID"

# Extract cluster ID and map to cell type
test_cluster_id <- gsub("[^0-9]", "", test_meta$cellID)
test_cellType_info <- test_anno[match(test_cluster_id, test_anno$`Cluster ID`), 3, drop = FALSE]
test_cellType_vector <- test_cellType_info$`Inferred cell type`

# Clean cellIDs for the test set
test_clean_cellID <- gsub("[0-9]", "", test_meta$cellID)
test_clean_cellID <- gsub(" ", "", test_clean_cellID)

# Create phenotype data frame for the test set
phenoData_test <- data.frame(
  cellType = test_cellType_vector,
  cellID = test_clean_cellID,
  stringsAsFactors = FALSE
)
rownames(phenoData_test) <- test_clean_cellID

# Standardize cell type names
phenoData_test$cellType <- gsub(" ", "", phenoData_test$cellType)
phenoData_test$cellType <- gsub("-", "", phenoData_test$cellType)
phenoData_test$cellID <- paste0("test", phenoData_test$cellID)
cat("\nCell type distribution in raw test data:\n")
print(table(phenoData_test$cellType))

# Load the sparse expression matrix for the test set
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set, ".sparse.RData"))
test_data <- sm
rm(sm)
cat("Dimensions of raw test expression matrix:", dim(test_data), "\n")
colnames(test_data) <- paste0("test", colnames(test_data))

# ----------------------------------------------------------------------------
# Step 4: Filter Test Data for Selected Cell Types
# ----------------------------------------------------------------------------
# Use the same list of cell types as for training
phenoData_test <- phenoData_test[phenoData_test$cellType %in% selected_cell_types, ]
phenoData_test <- phenoData_test[phenoData_test$cellID %in% colnames(test_data), ]
...
cat("Final dimensions of TEST data:\n")
cat("  Expression matrix (test_data):", dim(test_data), "\n")
cat("  Phenotype data (phenoData_test):", dim(phenoData_test), "\n")


# ----------------------------------------------------------------------------
# Step 5: Integrate Training and Test Data
# ----------------------------------------------------------------------------
# Standardize gene symbols to uppercase to ensure consistent matching
rownames(train_data) <- toupper(rownames(train_data))
rownames(test_data) <- toupper(rownames(test_data))

# Keep only the genes that are present in BOTH datasets
...
cat("\nGene intersection completed.\n")
cat("  Training data dimensions after intersection:", dim(train_data), "\n")
cat("  Test data dimensions after intersection:", dim(test_data), "\n")

# Combine the expression matrices column-wise
data <- as.matrix(cbind(train_data, test_data))
# Combine the phenotype data row-wise
full_phenoData <- rbind(phenoData_train, phenoData_test)

cat("\n=== FINAL INTEGRATED DATASET ===\n")
cat("Combined expression matrix (data):", dim(data), "\n")
cat("Combined phenotype data (full_phenoData):", dim(full_phenoData), "\n")
cat("Final cell type distribution:\n")
print(table(full_phenoData$cellType))

# Output the cell type counts for verification
train_counts <- table(phenoData_train$cellType)
test_counts <- table(phenoData_test$cellType)
cat("\nCell counts per type in training set:\n")
print(train_counts)
cat("\nCell counts per type in test set:\n")
print(test_counts)

# Remove temporary variables to clean the workspace (optional)
rm(train_anno, train_meta, cluster_id, cellType_info, cellType_vector, clean_cellID,
   test_anno, test_meta, test_cluster_id, test_cellType_info, test_cellType_vector,
   test_clean_cellID, common_genes, train_counts, test_counts)
# ============================================================================
...
#sim26 SRA481537_SRS1724938/ SRA694785_SRS3214440
prio_markers=list(
  Astrocytes =c("ABCB1A","EXOC3L","CLCA3A1","SLCO1A4","CAR4","LY6C1","LY6A"),
  Endothelialcells=c("ABCB1A","EXOC3L","CLCA3A1","SLCO1A4","CAR4","LY6C1","LY6A"),
  Microglia=c("CBR2","ZEB2OS","IL6RA","FCRLS","SIGLECH","FCGR1","CTS3","FCGR3"),
...
generator <- Generator(sce = as.matrix(test), phenoData = full_phenoData[full_phenoData$cellID %in% colnames(test), ], 
...
# ============================================================================
# EXPORT CIBERSORTX INPUT FILES (for manual run on cibersortx.stanford.edu)
# ============================================================================
# CIBERSORTx requires two tab-separated files of RAW integer counts: the signature
# matrix (gene x cell type) and the mixture file (gene x mixture sample). The first
# column must be GeneSymbol, no quoting, no NAs. C and T are still raw counts here
# (Transformation/Scaling happen on the lines below), so this is the right moment.
cibersortx_dir <- file.path(results_dir, "CIBERSORTx_input")
dir.create(cibersortx_dir, showWarnings = FALSE, recursive = TRUE)

sig_file <- file.path(cibersortx_dir, paste0(analysis_prefix, "_signature_matrix.txt"))
mix_file <- file.path(cibersortx_dir, paste0(analysis_prefix, "_mixture_file.txt"))

sig_out <- data.frame(GeneSymbol = rownames(C), as.matrix(C), check.names = FALSE)
write.table(sig_out, file = sig_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

mix_out <- data.frame(GeneSymbol = rownames(T), as.matrix(T), check.names = FALSE)
write.table(mix_out, file = mix_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

cat("========================================\n")
cat("CIBERSORTx input files exported to:\n  ", cibersortx_dir, "\n")
cat("  - ", basename(sig_file), " : ", nrow(C), " genes x ", ncol(C), " cell types\n", sep = "")
cat("  - ", basename(mix_file), " : ", nrow(T), " genes x ", ncol(T), " mixtures\n", sep = "")
cat("  cell types: ", paste(colnames(C), collapse = ", "), "\n", sep = "")
cat("Upload BOTH files to https://cibersortx.stanford.edu  ->  'Impute Cell Fractions'.\n")
cat("After the job finishes, save the downloaded Results CSV as:\n  ")
cat(file.path(cibersortx_dir, paste0(analysis_prefix, "_CIBERSORTx_Results.csv")), "\n")
cat("then re-run this script; the CIBERSORTx block below will read it automatically.\n")
cat("========================================\n")
...
# Load CIBERSORTx results from file (optional — skip gracefully if not produced yet)
cibersortx_result_csv <- file.path(cibersortx_dir, paste0(analysis_prefix, "_CIBERSORTx_Results.csv"))
if (file.exists(cibersortx_result_csv)) {
  cat("CIBERSORTx results CSV found, evaluating...\n")
  results <- read.table(cibersortx_result_csv, header = TRUE, sep = ",")
  ...
  cat("WARNING: CIBERSORTx results CSV not found at:\n  ", cibersortx_result_csv, "\n")
  cat("Skipping CIBERSORTx evaluation. To fill it in later:\n")
  cat("  1) Upload the two files in ", file.path(cibersortx_dir, ""), "\n")
  cat("     to https://cibersortx.stanford.edu  ->  'Impute Cell Fractions'.\n")
  cat("  2) Download the Results CSV and save it as the path above.\n")
  cat("  3) Re-run this script; this block will then evaluate CIBERSORTx automatically.\n")
}
...
cat("Selected cell types:", paste(selected_cell_types, collapse = ", "), "\n\n")
