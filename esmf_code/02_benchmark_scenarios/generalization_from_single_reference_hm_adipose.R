...
analysis_prefix <- "GeneralizationFromSingleReference_Hm_Adipose_threshold200"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA FROM HUSCH DATASETS
# ============================================================================
# This section loads and preprocesses human adipose tissue single-cell RNA-seq
# data from the HUSCH resource. The training data is from one donor sequenced
# on the 10x Genomics platform (GSE153643), while the test data consists of
# four donors from a different study (GSE134355). This setup follows a
# "sim52" design: same tissue (adipose), same species (human), same database
# (HUSCH), but different sequencing platforms/studies.
#
# Training dataset: HU_0303_Adipose_GSE153643
# Test datasets: HU_0271_Adipose_GSE134355, HU_0272_Adipose_GSE134355,
#                HU_0273_Adipose_GSE134355, HU_0274_Adipose_GSE134355
# Selected cell types: CD4T, Fibroblast, Mono_Macro
# ============================================================================

# Define a function to load a single HUSCH dataset (h5 format)
# This modular approach improves code readability and maintainability.
load_husch_adipose_dataset <- function(data_dir) {
  # Extract dataset identifier from the directory path
  dataset_id <- tail(strsplit(data_dir, "/")[[1]], 1)
  
  # Load metadata (cell type annotations and platform info)
  pheno_data <- read.csv(paste0(data_dir, dataset_id, "_meta.txt"),
                         sep = "\t", header = TRUE)
  
  # Standardize cell type names: replace problematic characters with underscores
  pheno_data$Celltype <- gsub("/", "_", pheno_data$Celltype)
  pheno_data$Celltype <- gsub(" ", "_", pheno_data$Celltype)
  colnames(pheno_data) <- c("cellID", "cellType", "Platform")
  
  # Load gene expression counts from HDF5 file
  count_matrix_h5 <- rhdf5::h5read(paste0(data_dir, dataset_id, "_gene_count.h5"),
                                   "matrix")
  
  # Construct a sparse matrix from the HDF5 components
  # This matches the structure of 10x Genomics output stored in HDF5.
  sparse_mat <- Matrix::sparseMatrix(
    i = count_matrix_h5$indices[] + 1,  # Convert from 0-based to 1-based indexing
    p = count_matrix_h5$indptr[],
    x = as.numeric(count_matrix_h5$data[]),
    dims = count_matrix_h5$shape[],
    repr = "T"
  )
  
  # Assign gene and cell barcodes as row and column names
  rownames(sparse_mat) <- count_matrix_h5$features$id
  colnames(sparse_mat) <- count_matrix_h5$barcodes
  
  # Convert to a standard dense matrix for downstream compatibility
  expr_mat <- as.matrix(sparse_mat)
  
  # Remove the sparse matrix to free memory
  rm(sparse_mat, count_matrix_h5)
  
  # Return both the expression matrix and phenotype dataframe
  return(list(expr = expr_mat, pheno = pheno_data))
}

# ----------------------------------------------------------------------------
# 3.1 Load and Process the Single Training Dataset
# ----------------------------------------------------------------------------
train_dir <- "/media/desk16/tjn050/LSC/HUSCH/Adipose/HU_0303_Adipose_GSE153643/"
train_loaded <- load_husch_adipose_dataset(train_dir)

train_data <- train_loaded$expr
phenoData_train <- train_loaded$pheno
rm(train_loaded)  # Clean up intermediate object

# Add a prefix to all cell IDs to uniquely identify them as training cells
colnames(train_data) <- paste0("train", colnames(train_data))
phenoData_train$cellID <- paste0("train", phenoData_train$cellID)

# Ensure consistency: keep only cells that are present in both metadata and matrix
...
# Display cell type composition for verification
cat("Training data - Cell type counts:\n")
print(table(phenoData_train$cellType))
cat(sprintf("Training data dimensions: %d genes x %d cells\n",
            nrow(train_data), ncol(train_data)))

# ============================================================================
# SECTION 4: LOAD AND PREPROCESS TEST DATA FROM MULTIPLE HUSCH DONORS
# ============================================================================
# Load four independent test datasets from the GSE134355 study.
# These will be merged to form a larger, more robust test set.
# ============================================================================

# List of directories for the four test datasets
test_dirs <- c(
...
# Initialize empty lists to store individual test datasets before merging
test_data_list <- list()
phenoData_test_list <- list()

for (idx in seq_along(test_dirs)) {
  dir_path <- test_dirs[idx]
  dataset_prefix <- paste0("test", idx)  # e.g., test1, test2...
  
  cat(sprintf("Loading test dataset %d from: %s\n", idx, dir_path))
  test_loaded <- load_husch_adipose_dataset(dir_path)
  
  temp_expr <- test_loaded$expr
  temp_pheno <- test_loaded$pheno
  rm(test_loaded)
  
  # Add a unique prefix to cell IDs for this specific test dataset
  colnames(temp_expr) <- paste0(dataset_prefix, colnames(temp_expr))
  temp_pheno$cellID <- paste0(dataset_prefix, temp_pheno$cellID)
  
  # Ensure cell ID consistency between expression matrix and metadata
  temp_pheno <- temp_pheno[temp_pheno$cellID %in% colnames(temp_expr), ]
  temp_expr <- temp_expr[, intersect(temp_pheno$cellID, colnames(temp_expr))]
  
  # Convert gene symbols to uppercase for consistent matching across datasets
  rownames(temp_expr) <- toupper(rownames(temp_expr))
  
  # Store the processed dataset
  test_data_list[[dataset_prefix]] <- temp_expr
  phenoData_test_list[[dataset_prefix]] <- temp_pheno
  
  cat(sprintf("  -> Loaded: %d genes x %d cells\n", nrow(temp_expr), ncol(temp_expr)))
  cat(sprintf("  -> Cell types: %s\n", paste(names(table(temp_pheno$cellType)), collapse=", ")))
}

# ----------------------------------------------------------------------------
# 4.1 Merge All Test Datasets
# ----------------------------------------------------------------------------
# Step 1: Find the common set of genes across all test datasets
common_genes_test <- Reduce(intersect, lapply(test_data_list, rownames))
cat(sprintf("\nNumber of common genes across all test datasets: %d\n", length(common_genes_test)))

# Step 2: Subset each dataset to the common genes and then column-bind them
test_data <- do.call(cbind, lapply(test_data_list, function(x) x[common_genes_test, ]))

# Step 3: Combine the phenotype data from all test datasets
phenoData_test <- do.call(rbind, phenoData_test_list)

# Remove the intermediate list objects to free memory
rm(test_data_list, phenoData_test_list)

cat(sprintf("Merged test data dimensions: %d genes x %d cells\n",
            nrow(test_data), ncol(test_data)))
cat("Merged test data - Cell type counts:\n")
print(table(phenoData_test$cellType))

# ----------------------------------------------------------------------------
# 4.2 Select Target Cell Types and Integrate Training & Test Data
# ----------------------------------------------------------------------------
# Define the three cell types of interest for adipose tissue deconvolution.
select_cellType <- c("CD4T", "Fibroblast", "Mono_Macro")

# Subset training data to selected cell types
phenoData_train <- phenoData_train[phenoData_train$cellType %in% select_cellType, ]
train_data <- train_data[, intersect(phenoData_train$cellID, colnames(train_data))]
cat(sprintf("\nTraining data after cell type selection: %d genes x %d cells\n",
            nrow(train_data), ncol(train_data)))
print(table(phenoData_train$cellType))

# Subset test data to selected cell types
...
cat(sprintf("Test data after cell type selection: %d genes x %d cells\n",
            nrow(test_data), ncol(test_data)))
print(table(phenoData_test$cellType))

# Ensure gene symbol case consistency for integration
rownames(train_data) <- toupper(rownames(train_data))
rownames(test_data) <- toupper(rownames(test_data))

# Find the intersection of genes between training and test sets
common_genes_final <- intersect(rownames(train_data), rownames(test_data))
train_data <- train_data[common_genes_final, ]
test_data <- test_data[common_genes_final, ]
cat(sprintf("\nNumber of common genes for final analysis: %d\n", length(common_genes_final)))

# ----------------------------------------------------------------------------
# 4.3 Create Combined Data Objects for Downstream Analysis
# ----------------------------------------------------------------------------
# Create the full combined matrix and phenotype data.
# This 'data' object is used for quality control in SECTION 5.
data <- as.matrix(cbind(train_data, test_data))
full_phenoData <- rbind(phenoData_train, phenoData_test)

# Final summary
cat("\n=== DATA INTEGRATION COMPLETE ===\n")
cat(sprintf("Combined 'data' matrix: %d genes x %d cells\n", nrow(data), ncol(data)))
cat("Final cell type distribution across all cells:\n")
print(table(full_phenoData$cellType))

# ============================================================================
# OUTPUT VARIABLES (for compatibility with subsequent sections)
# ============================================================================
# The following variables are now defined and match the structure expected by
# SECTION 5 and all subsequent analysis code:
#   - data: Combined expression matrix (genes x cells) for QC.
#   - full_phenoData: Combined phenotype data for all cells.
#   - train_data: Expression matrix for training/reference cells only.
#   - test_data: Expression matrix for test/validation cells only.
#   - phenoData_train: Phenotype data for training cells.
#   - phenoData_test: Phenotype data for test cells.
# ============================================================================


# ==============================================================================
# SECTION 5: Create Final Combined Objects
# ==============================================================================
# Combine training and test data into single matrix and phenotype object for
# subsequent quality control and analysis steps.
data <- cbind(train_data, test_data)
full_phenoData <- rbind(phenoData_train, phenoData_test)

cat("=== FINAL INTEGRATED DATA ===\n")
cat("Expression matrix dimensions (genes x cells):", dim(data), "\n")
cat("Phenotype data dimensions (rows x columns):", dim(full_phenoData), "\n")
cat("Final cell type distribution:\n")
print(table(full_phenoData$cellType))

# Clean up large intermediate objects (optional)
rm(train_data, test_data, phenoData_train, phenoData_test)
gc()
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_sim52_Results.csv", 
                      header = TRUE, sep = ",")
...
# ============================================================================
# SECTION 12: NON-NEGATIVE MATRIX FACTORIZATION DECONVOLUTION
# ============================================================================
