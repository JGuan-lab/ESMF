...
analysis_prefix <- "MultiSourceReferenceIntegration_Hm_Prostate_threshold200"
...
# ============================================================================
# This section loads and preprocesses five mouse prostate single-cell RNA-seq
# datasets from the PanglaoDB database (SRA accession: SRA742961).
# Training datasets: SRS3565197, SRS3565199, SRS3565201, SRS3565211, SRS3565210
# ============================================================================
...
anno_SRA_data1 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set1, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_data1) <- anno_SRA_data1[1, ]
anno_SRA_data1 <- anno_SRA_data1[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set1, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract cell type information based on cluster ID
...
# Add dataset-specific prefix to cell IDs
phenoData_train1$cellID <- paste0("train1", phenoData_train1$cellID)

# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set1, ".sparse.RData"))
train_data1 <- sm
rm(sm)  # Remove temporary object to free memory
dim(train_data1)

# Add prefix to column names (cell IDs) in expression matrix
...
# Convert sparse matrix to dense matrix and add metadata column
train_data1 <- as(train_data1, "matrix")
phenoData_train1$name <- phenoData_train1$cellID
dim(train_data1)
dim(phenoData_train1)

# Standardize gene names: use the first part before underscore
...
# Load cell type annotation file
anno_SRA_data2 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set2, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_data2) <- anno_SRA_data2[1, ]
anno_SRA_data2 <- anno_SRA_data2[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set2, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set2, ".sparse.RData"))
train_data2 <- sm
rm(sm)
dim(train_data2)
...
# Keep only genes common to both datasets
...
# Load cell type annotation file
anno_SRA_data3 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set3, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_data3) <- anno_SRA_data3[1, ]
anno_SRA_data3 <- anno_SRA_data3[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set3, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set3, ".sparse.RData"))
train_data3 <- sm
rm(sm)
dim(train_data3)
...
# Prepare for merging: convert gene names to uppercase and filter to common genes
...
# Load cell type annotation file
anno_SRA_data4 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set4, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_data4) <- anno_SRA_data4[1, ]
anno_SRA_data4 <- anno_SRA_data4[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set4, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set4, ".sparse.RData"))
train_data4 <- sm
rm(sm)
dim(train_data4)
...
# Merge with existing combined training data
...
# Load cell type annotation file
anno_SRA_data5 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set5, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_data5) <- anno_SRA_data5[1, ]
anno_SRA_data5 <- anno_SRA_data5[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set5, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set5, ".sparse.RData"))
train_data5 <- sm
rm(sm)
dim(train_data5)
...
# Final merge of all five training datasets
...
# ============================================================================
# Load test dataset: HUSCH Prostate (GSE145843)
# This dataset serves as independent validation for deconvolution methods.
# Data is stored in HDF5 format with associated metadata.
# ============================================================================

# Define directory containing the HUSCH test dataset
test_dir <- "/media/desk16/tjn050/LSC/HUSCH/Prostate/HU_0242_Prostate_GSE145843/"
test_set <- strsplit(test_dir, "/")[[1]][8]  # Extract dataset identifier
test_set

# Load metadata for the test dataset
...
# Rename metadata columns to match expected format
colnames(phenoData_test) <- c("cellID", "cellType", "Platform")
table(phenoData_test$cellType)

# Load expression data from HDF5 file
...
# Convert to dense matrix for compatibility with downstream functions
test_data <- as.matrix(mat)
rm(mat)  # Remove temporary sparse matrix

# Add prefix to test dataset cell IDs
...
# ----------------------------------------------------------------------------
# Harmonize Cell Type Names Between Training and Test Data
# ----------------------------------------------------------------------------
# Standardize test dataset cell type names to match training data conventions
...
# Define cell types to retain for analysis (prostate-relevant)
...
phenoData_test <- phenoData_test[, c(2, 1, 4)]  # Reorder columns: cellType, cellID, name
table(phenoData_test$cellType)

dim(test_data)
dim(phenoData_test)

# Standardize gene names in training data
...
#sim4 Prostate (人类同组织同测序方法)
prio_markers=list(
  Basalcells =c("CYR61","DST","KRT14","KRT15","TAGLN"),
  Endothelialcells =c("ACKR1","CLDN5","HMOX1","IFI27","SELE"),
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/sim4_SRA742961_SRS3565197_CIBERSORTx_Job110_Results.csv", 
                      header = TRUE, sep = ",")
