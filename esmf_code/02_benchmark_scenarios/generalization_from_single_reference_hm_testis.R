...
analysis_prefix <- "GeneralizationFromSingleReference_Hm_Testis_threshold200"
...
# ============================================================================
# This section loads and preprocesses a single human testis single-cell RNA-seq dataset
# from the PanglaoDB database (SRA826293_SRS4181130).
# The dataset serves as the training reference for deconvolution.
# Selected cell types: Dendritic cells, Endothelial cells, Germ cells
# ============================================================================

# ----------------------------------------------------------------------------
# Training Dataset: SRA826293_SRS4181130
# ----------------------------------------------------------------------------
set1 = "SRA826293_SRS4181130"
...
# Prepare for integration: Convert gene names to uppercase for consistency
rownames(train_data1) <- toupper(rownames(train_data1))

# Assign to final training variables (to match downstream variable names)
train_data <- train_data1
phenoData_train <- phenoData_train1
dim(train_data)
dim(phenoData_train)
table(phenoData_train$cellType)

# Remove temporary object
rm(train_data1, phenoData_train1)

# ============================================================================
# SECTION 4: LOAD AND PREPROCESS TEST DATA FROM THREE DATASETS
# ============================================================================
# Load and merge three test datasets from the same study (PanglaoDB: SRA826293).
# These datasets serve as independent validation for deconvolution methods.
# Test datasets: SRA826293_SRS4181129, SRA826293_SRS4181128, SRA826293_SRS4181127
# ============================================================================

# ----------------------------------------------------------------------------
# Test Dataset 1: SRA826293_SRS4181129
# ----------------------------------------------------------------------------
test_set1 = "SRA826293_SRS4181129"
...
# ----------------------------------------------------------------------------
# Test Dataset 2: SRA826293_SRS4181128
# ----------------------------------------------------------------------------
test_set2 = "SRA826293_SRS4181128"
...
# ----------------------------------------------------------------------------
# Convert gene names to uppercase and keep common genes
...
# ----------------------------------------------------------------------------
# Test Dataset 3: SRA826293_SRS4181127
# ----------------------------------------------------------------------------
test_set3 = "SRA826293_SRS4181127"
...
# ----------------------------------------------------------------------------
# For this testis tissue analysis, we focus on three major cell types:
# Dendritic cells, Endothelial cells, and Germ cells
select_cellType <- c("Dendriticcells", "Endothelialcells", "Germcells")
...
# Clean up intermediate variables to free memory (as done in original script)
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_Job123_sim51_Results.csv", 
                      header = TRUE, sep = ",")
...
cat("Analysis date:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("Analysis prefix:", analysis_prefix, "\n")
# cat("Training datasets:", paste(c(set1, set2, set3, set4), collapse = ", "), "n")
cat("Test datasets:", paste(c(test_set1, test_set2, test_set3), collapse = ", "), "\n")
