...
analysis_prefix <- "GeneralizationFromSingleReference_Mm_Brain_threshold200"
...
# ============================================================================
# SECTION 3: LOAD AND PREPROCESS TRAINING DATA (sim5 SCENARIO)
# ============================================================================
# This section loads and preprocesses the TRAINING dataset for the sim5 scenario.
# Scenario: Mouse brain, same tissue, same database (PanglaoDB), same sequencing platform.
# Training dataset: SRA751294_SRS3625047
# Target cell types: Astrocytes, Endothelial cells, Microglia
# ============================================================================

# Define the training dataset identifier
set <- "SRA751294_SRS3625047"

# Load cell type annotation file
anno_SRA_data <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set, "_annotation.csv"),
                          sep = ",", header = FALSE)
colnames(anno_SRA_data) <- anno_SRA_data[1, ]
anno_SRA_data <- anno_SRA_data[-1, ]

# Load cluster assignment file
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract clean cell type and cell ID information
...
# Create phenotype data frame for training set
...
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", set, ".sparse.RData"))
train_data <- sm
...
# Convert sparse matrix to dense matrix and add metadata
train_data <- as(train_data, "matrix")
phenoData_train$name <- phenoData_train$cellID
dim(train_data)
dim(phenoData_train)

# Standardize gene names: keep the part before the first underscore
rownames(train_data) <- unlist(lapply(strsplit(rownames(train_data), "_"), function(x) x[1]))

# ============================================================================
# SECTION 4: LOAD AND PREPROCESS TEST DATA (sim5 SCENARIO)
# ============================================================================
# This section loads and preprocesses the TEST datasets for the sim5 scenario.
# Test datasets: SRA670243_SRS3078084, SRA667466_SRS3060015, SRA667466_SRS3060012
# Target cell types: Astrocytes, Endothelial cells, Microglia
# The three test datasets are merged to form a larger, independent validation set.
# ============================================================================

# ----------------------------------------------------------------------------
# Test Dataset 1: SRA670243_SRS3078084
# ----------------------------------------------------------------------------
test_set1 <- "SRA670243_SRS3078084"

# Load annotation
anno_SRA_test1 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set1, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_test1) <- anno_SRA_test1[1, ]
anno_SRA_test1 <- anno_SRA_test1[-1, ]

# Load cluster assignments
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set1, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract clean cell type and cell ID
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
...
# Create phenotype data frame for test set 1
...
# Load sparse expression matrix
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set1, ".sparse.RData"))
test_data1 <- sm
rm(sm)
dim(test_data1)
...
# ----------------------------------------------------------------------------
# Test Dataset 2: SRA667466_SRS3060015
# ----------------------------------------------------------------------------
test_set2 <- "SRA667466_SRS3060015"

# Load annotation
anno_SRA_test2 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set2, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_test2) <- anno_SRA_test2[1, ]
anno_SRA_test2 <- anno_SRA_test2[-1, ]

# Load cluster assignments
meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set2, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set2, ".sparse.RData"))
test_data2 <- sm
rm(sm)
dim(test_data2)
...
# ----------------------------------------------------------------------------
# Ensure gene name consistency and keep only common genes
...
# ----------------------------------------------------------------------------
# Test Dataset 3: SRA667466_SRS3060012
# ----------------------------------------------------------------------------
test_set3 <- "SRA667466_SRS3060012"

# Load annotation
anno_SRA_test3 <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set3, "_annotation.csv"),
                           sep = ",", header = FALSE)
colnames(anno_SRA_test3) <- anno_SRA_test3[1, ]
anno_SRA_test3 <- anno_SRA_test3[-1, ]

meta_temp <- read.csv(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set3, ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"
...
load(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set3, ".sparse.RData"))
test_data3 <- sm
rm(sm)
dim(test_data3)
...
# ----------------------------------------------------------------------------
# Final Merge of All Test Datasets (1, 2, and 3)
# ----------------------------------------------------------------------------
# Merge the previously combined test data (1+2) with the new test dataset 3
...
# ----------------------------------------------------------------------------
# Focus on three major brain cell types for deconvolution benchmarking
select_cellType <- c("Astrocytes", "Endothelialcells", "Microglia")
...
# ----------------------------------------------------------------------------
# Final Data Integration (Training + Test)
# ----------------------------------------------------------------------------
# Ensure gene name consistency and keep only genes present in both training and test sets
...
# Combine all data into a single matrix for downstream quality control and splitting
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/CIBERSORTx_sim55_Results.csv", 
                      header = TRUE, sep = ",")
