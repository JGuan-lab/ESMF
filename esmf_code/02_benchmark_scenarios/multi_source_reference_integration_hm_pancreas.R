...
analysis_prefix <- "MultiSourceReferenceIntegration_Hm_Pancreas_threshold200"
...
# ============================================================================
# This section loads and preprocesses four human pancreas single-cell RNA-seq
# datasets from the HUSCH database. The datasets share the same tissue (pancreas)
# and species (human) but are from different studies and platforms.
# Training datasets: HU_0230_Pancreas_EBI, HU_0231_Pancreas_GSE134355,
#                    HU_0228_Pancreas_GSE84133, HU_0229_Pancreas_GSE85241
# Test dataset: SRA701877_SRS3279695 (from PanglaoDB)
# Selected cell types: Alphacells, Betacells, Ductalcells
# ============================================================================

# ----------------------------------------------------------------------------
# Training Dataset 1: HU_0230_Pancreas_EBI
# ----------------------------------------------------------------------------
train_dir1 <- "/media/desk16/tjn050/LSC/HUSCH/Pancreas/HU_0230_Pancreas_EBI/"
train_set1 <- strsplit(train_dir1, "/")[[1]][8]

# Load metadata file containing cell type information
phenoData_train1 <- read.csv(paste0(train_dir1, train_set1, "_meta.txt"), 
                             sep = "\t", header = TRUE) 

# Standardize cell type names: replace special characters with underscores
phenoData_train1$Celltype <- sub("/", "_", phenoData_train1$Celltype)
phenoData_train1$Celltype <- sub(" ", "_", phenoData_train1$Celltype)
phenoData_train1$Celltype <- sub(" ", "_", phenoData_train1$Celltype)
...
# Load sparse matrix from HDF5 file
train_mat1 <- h5read(paste0(train_dir1, train_set1, "_gene_count.h5"), "matrix")  

# Extract components for sparse matrix reconstruction
...
train_data1 <- as.matrix(mat)

# Clean up memory
rm(mat, train_mat1)

# Add dataset-specific prefix to cell identifiers
...
# ----------------------------------------------------------------------------
# Training Dataset 2: HU_0231_Pancreas_GSE134355
# ----------------------------------------------------------------------------
train_dir2 <- "/media/desk16/tjn050/LSC/HUSCH/Pancreas/HU_0231_Pancreas_GSE134355/"
train_set2 <- strsplit(train_dir2, "/")[[1]][8]

phenoData_train2 <- read.csv(paste0(train_dir2, train_set2, "_meta.txt"), 
                             sep = "\t", header = TRUE) 

# Standardize cell type names
phenoData_train2$Celltype <- sub("/", "_", phenoData_train2$Celltype)
phenoData_train2$Celltype <- sub(" ", "_", phenoData_train2$Celltype)
phenoData_train2$Celltype <- sub(" ", "_", phenoData_train2$Celltype)
...
train_data2 <- train_data2[intersect(rownames(train_data2), rownames(train_data1)), ]
dim(train_data1)
dim(train_data2)
...
# ----------------------------------------------------------------------------
# Training Dataset 3: HU_0228_Pancreas_GSE84133
# ----------------------------------------------------------------------------
train_dir3 <- "/media/desk16/tjn050/LSC/HUSCH/Pancreas/HU_0228_Pancreas_GSE84133/"
train_set3 <- strsplit(train_dir3, "/")[[1]][8]

phenoData_train3 <- read.csv(paste0(train_dir3, train_set3, "_meta.txt"), 
                             sep = "\t", header = TRUE) 

# Standardize cell type names
phenoData_train3$Celltype <- sub("/", "_", phenoData_train3$Celltype)
phenoData_train3$Celltype <- sub(" ", "_", phenoData_train3$Celltype)
phenoData_train3$Celltype <- sub(" ", "_", phenoData_train3$Celltype)
...
# ----------------------------------------------------------------------------
# Merge Training Dataset 3 with Existing Data
# ----------------------------------------------------------------------------
# Convert gene names to uppercase for consistency
...
train_data <- train_data[intersect(rownames(train_data3), rownames(train_data)), ]
train_data3 <- train_data3[intersect(rownames(train_data3), rownames(train_data)), ]
dim(train_data)
dim(train_data3)
...
# Clean up memory
rm(phenoData_train3, train_data3)
...
# ----------------------------------------------------------------------------
# Training Dataset 4: HU_0229_Pancreas_GSE85241
# ----------------------------------------------------------------------------
train_dir4 <- "/media/desk16/tjn050/LSC/HUSCH/Pancreas/HU_0229_Pancreas_GSE85241/"
train_set4 <- strsplit(train_dir4, "/")[[1]][8]

phenoData_train4 <- read.csv(paste0(train_dir4, train_set4, "_meta.txt"), 
                             sep = "\t", header = TRUE) 

# Standardize cell type names
phenoData_train4$Celltype <- sub("/", "_", phenoData_train4$Celltype)
phenoData_train4$Celltype <- sub(" ", "_", phenoData_train4$Celltype)
phenoData_train4$Celltype <- sub(" ", "_", phenoData_train4$Celltype)
...
# Keep only common genes
train_data <- train_data[intersect(rownames(train_data4), rownames(train_data)), ]
train_data4 <- train_data4[intersect(rownames(train_data4), rownames(train_data)), ]
dim(train_data)
dim(train_data4)
...
# Add name column and reorder columns for consistency
phenoData_train$name <- phenoData_train$cellID
phenoData_train <- phenoData_train[, c(2, 1, 4)]  # Reorder columns: cellType, cellID, name
table(phenoData_train$cellType)
dim(train_data)
dim(phenoData_train)
...
# ============================================================================
# Load test dataset: SRA701877_SRS3279695 from PanglaoDB
# This dataset serves as independent validation for deconvolution methods
# ============================================================================

test_set <- "SRA701877_SRS3279695"

# Load annotation file
anno_SRA_test <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set), "_annotation.csv"), 
                          sep = ",", header = FALSE)
colnames(anno_SRA_test) <- anno_SRA_test[1, ]
anno_SRA_test <- anno_SRA_test[-1, ]

# Load cluster assignments
meta_temp <- read.csv(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set), ".clusters.txt"),
                      sep = "\t", header = FALSE)
colnames(meta_temp) <- "cellID"

# Extract cell type information
cellType_temp <- gsub("[^0-9]", "", meta_temp$cellID)
cellType_temp <- anno_SRA_test[match(cellType_temp, anno_SRA_test$`Cluster ID`), 3, drop = FALSE]
...
phenoData_test <- data.frame(cellType = cellType_data, cellID = cellID_data) 
rownames(phenoData_test) <- cellID_data

# Clean cell type names: remove spaces and hyphens
phenoData_test$cellType <- sub(" ", "", phenoData_test$cellType)
phenoData_test$cellType <- sub(" ", "", phenoData_test$cellType)
phenoData_test$cellType <- sub(" ", "", phenoData_test$cellType)
phenoData_test$cellType <- sub(" ", "", phenoData_test$cellType)
phenoData_test$cellType <- sub("-", "", phenoData_test$cellType)
...
load(paste0(paste0("/media/desk16/tjn050/LSC/PanglaoDB/", test_set), ".sparse.RData"))
test_data <- sm
...
select_cellType <- gsub("Unknown", "", unique(phenoData_test$cellType))
...
# ----------------------------------------------------------------------------
# Harmonize cell type names between HUSCH and PanglaoDB datasets
phenoData_train$cellType <- sub("Acinar", "Acinarcells", phenoData_train$cellType)
phenoData_train$cellType <- sub("Alpha", "Alphacells", phenoData_train$cellType)
phenoData_train$cellType <- sub("Beta", "Betacells", phenoData_train$cellType)
phenoData_train$cellType <- sub("Pancreatic_Stellate", "Pancreaticstellatecells", phenoData_train$cellType)
phenoData_train$cellType <- sub("Duct", "Ductalcells", phenoData_train$cellType)

# ----------------------------------------------------------------------------
# Select Specific Cell Types for Analysis
# ----------------------------------------------------------------------------
# For this pancreas tissue analysis, we focus on three major endocrine cell types:
# Alphacells, Betacells, and Ductalcells
select_cellType <- c("Alphacells","Betacells","Ductalcells")
...
# source("test_generator_input.R")
# test_generator_detailed(sce = test, phenoData = full_phenoData, 
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/sim44_CIBERSORTx_Job133_Results.csv", 
                      header = TRUE, sep = ",")
