...
analysis_prefix <- "Ablation_Study_crossPlatformConsistency_Mm_Brain"
ablation_version <- "v3"
...
# 仅追加到此副本脚本 Ablation_Study_cross-platform-consistency-hm-bone-marrow.R。
...
fit_signature <- function(md_obj) {
  ...
  for (i in seq_along(ct_names)) {
    ...
    for (j in seq_len(nrow(counts))) {
      if (length(which(round(counts[j, ]) != 0)) > 0) {
        f1 <- fitdist(round(counts[j, ]), "nbinom", method = "mse")
        # 修复：fitdist 对 nbinom 返回 estimate 顺序为 c(size, mu)，必须按名字取，
        #       否则 mu/size 互换（与正文第 716-741 行用 [["mu"]]/[["size"]] 的口径一致）。
