...
analysis_prefix <- "Ablation_Study_crossPlatformConsistency_Mm_Brain_ParameterSensitivity_v2"
ablation_version <- "v2"
...
# ============================================================================
# This section loads and preprocesses mouse brain single-cell RNA-seq data
# from PanglaoDB datasets. The datasets are from the same species (mouse) and
# tissue (brain) but different studies. Training dataset: SRA866994_SRS4545961
# Selected cell types: Macrophages, Microglia, T memory cells
# ============================================================================

# ----------------------------------------------------------------------------
# Training Dataset: SRA866994_SRS4545961
# ----------------------------------------------------------------------------
# Load annotation file containing cell type information
anno_SRA_data <- read.csv("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545961_annotation.csv", 
                          sep = ",", header = FALSE) 
colnames(anno_SRA_data) <- anno_SRA_data[1,]
anno_SRA_data <- anno_SRA_data[-1,]

# Load cluster assignment file
meta_temp <- read.csv("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545961.clusters.txt",
                      sep = "\t", header = FALSE) 
...
load("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545961.sparse.RData")
train_data <- sm
...
select_cellType <- c("Macrophages", "Microglia", "Tmemorycells")
...
# ============================================================================
# Load test dataset: SRA866994_SRS4545964
# This dataset serves as independent validation for deconvolution methods
# ============================================================================

# Load annotation file
anno_SRA_data <- read.csv("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545964_annotation.csv", 
                          sep = ",", header = FALSE) 
colnames(anno_SRA_data) <- anno_SRA_data[1,]
anno_SRA_data <- anno_SRA_data[-1,]

# Load cluster assignments
meta_temp <- read.csv("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545964.clusters.txt",
                      sep = "\t", header = FALSE) 
colnames(meta_temp) <- "cellID"
...
# Load sparse expression matrix
load("/media/desk16/tjn050/LSC/PanglaoDB/SRA866994_SRS4545964.sparse.RData")
test_data <- sm
rm(sm)
dim(test_data)
...
# Select specific cell types for analysis (same as training)
select_cellType <- c("Macrophages", "Microglia", "Tmemorycells")
...
#sim25 SRA866994_SRS4545961/ SRA866994_SRS4545964
prio_markers=list(
  # Bcells=c(     "H2-DMB2",
  #               "H2-OB",
  #               "SIGLECG",
...
if (FALSE) {  # 消融/调参脚本：跳过 single-cell 方法（MuSiC/CIBERSORTx）
...
results <- read.table("/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/sim25_CIBERSORTx_Job134_Results.csv", 
                      header = TRUE, sep = ",")
...
baseline_methods <- list(
  nnls  = c(RMSE = 0.1826, Pearson = 0.7148),
  OLS   = c(RMSE = 0.1803, Pearson = 0.7216),
  ridge = c(RMSE = 0.1984, Pearson = 0.6438))
...
baseline_ts <- "20260528_172043"   # 正文基准 ESMF 参数 timestamp（ESMF 基准 0.0951/0.9279 那次）
baseline_param_dir <- file.path(
  "/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/results/crossPlatformConsistency_Mm_Brain",
  "ESMF_parameters")
...
# 仅追加到此副本脚本 Ablation_Study_crossPlatformConsistency_Mm_Brain_ParameterSensitivity_v2.R。
# 正文基准（cross-platform-consistency-mm-brain.R）保持不动。
# 单变量控制铁律：每一组只改一个变量。
...
fit_signature <- function(md_obj, para = c(1, 1, 1)) {
  ...
  for (i in seq_along(ct_names)) {
    ...
    for (j in seq_len(nrow(counts))) {
      if (length(which(round(counts[j, ]) != 0)) > 0) {
        f1 <- tryCatch(
          fitdist(round(counts[j, ]), "nbinom", method = "mse"),
...
p1_out <- lapply(p1_grid, function(fac) {
  ...
  tryCatch(
    ...
    error = function(e) {
      cat("  [ERROR] rate factor =", fac, ":", conditionMessage(e), "\n")
      c(label = paste0("P1_rate_", fac), iter = 3000, seed = 1233322,
        H_RMSE = NA, H_Pearson = NA, W_RMSE = NA, W_Pearson = NA)
    })
})
