# ============================================================================
# sweep-esmf-hm-adipose.R —— 人脂肪数据 ESMF 超参数遍历（sweep，真实测试集评估）
#
# 做法：
#   1. source 主脚本到 SECTION 9 之前：
#      - 训练集（GSE134355: HU_0271~0274_Adipose 多来源合并）构建参考对象；
#      - 真实测试集（HU_0303_Adipose_GSE153643）单细胞经 Generator 合成伪 bulk，
...
cat("========================================\n")
cat("脚本版本：sweep-esmf-hm-adipose.R (2026-09-09 sweep)\n")
...
MAIN_SCRIPT <- "/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/Ablation_Parameter_Codes/Ablation_Study_MultiSourceReferenceIntegration_Hm_Adipose_ParameterSensitivity_v2.R"
...
cat(">> 完成。\n")
cat("   参考 sig_matrix:", nrow(sig_matrix), "基因 x", ncol(sig_matrix), "细胞类型（训练集 GSE134355 构建）\n")
cat("   真实测试集 V:", nrow(V), "基因 x", ncol(V), "样本（测试集 HU_0303_Adipose_GSE153643 合成伪 bulk）\n")
