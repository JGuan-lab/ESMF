# ============================================================================
# sweep-esmf-hm-pancreas.R —— 人胰腺数据 ESMF 超参数遍历（sweep，真实测试集评估）
#
# 做法：
#   1. source 主脚本到 SECTION 9 之前：
#      - 训练集（HU_0228_Pancreas_GSE84133）构建参考对象；
#      - 真实测试集（SRA701877: SRS3279695 + SRS3279693 + SRS3279692 合并）
#        单细胞经 Generator 合成伪 bulk，得到评估用的 V（测试伪 bulk）与 P（测试真比例）。
#   2. 调用 esmf_sweep()：para1 固定=1，遍历 rate × para2 × para3，
#      在真实测试集 V/P 上用 PCC.C 选参。
# ============================================================================

cat("========================================\n")
cat("脚本版本：sweep-esmf-hm-pancreas.R (2026-09-09 sweep)\n")
...
MAIN_SCRIPT <- "/media/desk16/tjn050/LSC/LSC_deconvolution/LSC/Ablation_Parameter_Codes/Ablation_Study_GeneralizationFromSingleReference_Hm_Pancreas_ParameterSensitivity_v2.R"
...
cat(">> 完成。\n")
cat("   参考 sig_matrix:", nrow(sig_matrix), "基因 x", ncol(sig_matrix), "细胞类型（训练集 HU_0228_Pancreas_GSE84133 构建）\n")
cat("   真实测试集 V:", nrow(V), "基因 x", ncol(V), "样本（测试集 SRA701877 合成伪 bulk）\n")
